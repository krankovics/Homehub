...
type NetworkStatus = { id:string; name:string; kind:string; online:boolean; adminOnline?:boolean; ip:string; configuredIp?:string; ipSource?:string; ipChanged?:boolean; mac:string; latencyMs:number; adminUrl:string; note:string; managed?:NetworkManagedStatus };
...
function NetworkDeviceCard({n}:{n:NetworkStatus}){
  return <article className={`networkDevice ${n.online?"online":"offline"}`}><div className="deviceHead"><div><span className={`deviceDot ${n.online?"on":""}`}></span><strong>{n.name}</strong></div><span className="deviceKind">{networkKind(n.kind)}</span></div><small>{n.ip||"IP keresése…"}{n.mac?` · ${n.mac}`:""}</small>{n.ipChanged&&<div className="ipChangeBadge">DHCP/IP változás · konfigurált: {n.configuredIp}</div>}<div className="networkMeta"><span>{n.online?(n.latencyMs>0?`${n.latencyMs.toFixed(1)} ms`:"Jelen van"):"Nem elérhető"}</span>{n.ipSource==="arp-mac"&&<span>MAC → IP</span>}{n.adminUrl&&n.ip&&<a className={n.adminOnline===false?"mutedLink":""} href={n.adminUrl} target="_blank" rel="noreferrer">Admin{n.adminOnline===false?" (nem válaszol)":""}</a>}</div><em>{n.note}</em>{n.managed&&<small className={n.managed.authOk?"managedMini ok":"managedMini"}>{n.managed.authOk?`${n.managed.ports?.filter(p=>p.linkUp).length||0} aktív switch port`:n.managed.error||"Részletes integráció nincs kész"}</small>}</article>
}
