...
type SmartFilter = "all"|"switch"|"sensor"|"climate"|"light"|"gate"|"device";
...
function labelKind(kind:SmartFilter){return kind==="climate"?"Klíma":kind==="sensor"?"Szenzor":kind==="switch"?"Kapcsoló":kind==="light"?"Világítás":kind==="gate"?"Kapu":"Eszköz"}
...
function SmartDeviceCard({device,onCommand}:{device:TuyaDevice;onCommand:(d:TuyaDevice,code:string,value:unknown)=>void}){
  ...
  return <article className={`smartDevice ${kind} ${device.online?"online":"offline"}`}>
    ...
    <div className="deviceControls">
      ...
      {modeCode&&modeRange.length>0&&<label className="selectControl">Mód<select value={String(sm[modeCode]??"")} disabled={!device.online} onChange={e=>onCommand(device,modeCode,e.target.value)}>{modeRange.map((m:string)=><option value={m} key={m}>{m}</option>)}</select></label>}
      {fanCode&&fanRange.length>0&&<label className="selectControl">Ventilátor<select value={String(sm[fanCode]??"")} disabled={!device.online} onChange={e=>onCommand(device,fanCode,e.target.value)}>{fanRange.map((m:string)=><option value={m} key={m}>{m}</option>)}</select></label>}
    </div>
...
function TopoNode({title,subtitle,status,kind="node"}:{title:string;subtitle?:string;status?:NetworkStatus;kind?:string}){
  ...
  return <div className={`topoNode ${kind} ${known?(on?"online":"offline"):"passive"}`}><div className="topoNodeTitle"><span className={`deviceDot ${on?"on":""}`}></span><strong>{title}</strong></div>{subtitle&&<small>{subtitle}</small>}{status&&<span className="topoState">{status.online?(status.ip||"Online"):(status.ip?"Nem válaszol":"IP keresése…")}</span>}</div>
}
...
function NetworkTopology({network}:{network:NetworkStatus[]}){
  ...
  return <div className="topologyWrap">
    ...
    <div className="topology">
      ...
      <div className="topoBranches">
        <section className="topoLane"><header><span className="laneBadge wifi">Wi-Fi</span><strong>krankovics2</strong></header><div className="topoLaneBody single"><TopoNode title="Krankovics-MBP" subtitle="2.4 GHz · 192.168.1.114" status={byId["krankovics-mbp"]} kind="computer"/></div></section>
        <section className="topoLane ethernet"><header><span className="laneBadge ethernet">Ethernet</span><strong>Vezetékes hálózat</strong></header><div className="portGrid">
          <div className="portColumn"><div className="portLabel">Port 1</div><TopoNode title="DESKTOP-E6K3SEK" subtitle="192.168.1.25" status={byId["desktop-e6k3sek"]} kind="computer"/></div>
          <div className="portColumn wide"><div className="portLabel">Port 2</div><TopoNode title="TL-SG108E" subtitle="Menedzselhető switch · 192.168.1.49" status={byId["tl-sg108e"]} kind="switch"/><div className="downstreamGrid">
            <TopoNode title="DorkaPC" subtitle="192.168.1.210" status={byId["dorkapc"]} kind="computer"/>
            <div className="topoChain"><TopoNode title="D-Link GO-SW-5G" subtitle="Passzív gigabit switch" kind="switch"/><span className="chainArrow">↓</span><TopoNode title="TP-Link LiteWave LS105G" subtitle="Passzív gigabit switch" kind="switch"/><span className="chainArrow">↓</span><TopoNode title="davidgaming" subtitle="192.168.1.138" status={byId["davidgaming"]} kind="computer"/></div>
            <div className="meshBranch"><TopoNode title="Archer C6" subtitle="Mesh főpont · 192.168.1.129" status={byId["archer-c6"]} kind="router"/><div className="ssidLabel">Wi-Fi / mesh: <strong>krankovics</strong></div><div className="meshNodes"><TopoNode title="RE220" status={byId["re220"]} kind="extender"/><TopoNode title="RE315 #1" status={byId["re315-1"]} kind="extender"/><TopoNode title="RE315 #2" status={byId["re315-2"]} kind="extender"/></div></div>
          </div></div>
          <div className="portColumn"><div className="portLabel">Port 3</div><TopoNode title="KD20 / oldnas" subtitle="Torrent NAS · 192.168.1.12" status={byId["kd20"]} kind="nas"/></div>
          <div className="portColumn"><div className="portLabel">Port 4</div><TopoNode title="WD My Cloud" subtitle="Médiatár · 192.168.1.180" status={byId["wd-my-cloud"]} kind="nas"/></div>
        </div></section>
      </div>
    </div>
  </div>
...
function App(){
  const[auth,setAuth]=useState<"checking"|"yes"|"no">("checking"),[state,setState]=useState<State|null>(null),[magnet,setMagnet]=useState(""),[busy,setBusy]=useState(false),[notice,setNotice]=useState(""),[deleteTarget,setDeleteTarget]=useState<Torrent|null>(null),[tab,setTab]=useState<Tab>(initialTab),[smartFilter,setSmartFilter]=useState<SmartFilter>("all");
