...
type PersonProfile = {id:string;name:string;nickname?:string;role?:string;devices:PersonDeviceLink[];createdAt:string;updatedAt:string;hasAvatar?:boolean};
...
function PeopleTab({people,presence,network,reload,flash}:{people:PersonProfile[];presence:PresenceStatus[];network:NetworkStatus[];reload:()=>void;flash:(m:string)=>void}){
  ...
  function reset(){setEditing(null);setCreating(false);setName("");setNickname("");setRole("Családtag");setLinks({})}
  function startNew(){reset();setCreating(true)}
  function startEdit(p:PersonProfile){setEditing(p);setCreating(true);setName(p.name);setNickname(p.nickname||"");setRole(p.role||"Családtag");setLinks(Object.fromEntries(p.devices.map(d=>[d.networkId,d.role])))}
  async function save(){if(!name.trim())return;setBusy(true);try{const devices=Object.entries(links).filter(([,r])=>Boolean(r)).map(([networkId,r])=>({networkId,role:r as PersonDeviceRole,label:network.find(n=>n.id===networkId)?.name||""}));const body={name:name.trim(),nickname:nickname.trim(),role:role.trim(),devices};if(editing)await api(`/api/people/${editing.id}`,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify(body)});else await api("/api/people",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});flash(editing?"Profil frissítve":"Személy felvéve");reset();reload()}catch(err){flash(`Profil hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}finally{setBusy(false)}}
  ...
  return <div className="tabPanel peopleTabV19"><section className="panel peopleHeroV19"><div><span className="smartEyebrowV12">HOUSEHOLD</span><h2>Emberek és jelenlét</h2><p>Kapcsold össze a családtagokat a hálózati eszközeikkel. Telefon legyen elsődleges jelenléti eszköz; laptop vagy PC inkább másodlagos/stacionárius.</p></div><button className="primaryAction" onClick={startNew}>+ Személy felvétele</button></section>
  <section className="peopleGridV19">{people.map(p=>{const ps=byPresence.get(p.id);return <article className="panel personCardV19" key={p.id}><div className="personHeadV19"><PersonAvatar person={p} size="large"/><div><strong>{p.name}</strong><span>{p.role||"Családtag"}</span><b className={`presenceBadgeV19 ${ps?.status||"uncertain"}`}>{ps?.status==="home"?"Itthon":ps?.status==="away"?"Nincs itthon":"Bizonytalan"} · {ps?.confidence??0}%</b></div></div><div className="personMetaV19"><span><small>Észlelés</small><b>{ps?.source||"—"}</b></span><span><small>Állapot kezdete</small><b>{ps?.since?new Date(ps.since).toLocaleString("hu-HU"):"—"}</b></span></div><div className="personDevicesV19">{p.devices.length?p.devices.map(d=>{const n=network.find(x=>x.id===d.networkId);return <span key={d.networkId}><i className={n?.online?"online":""}></i>{d.label||n?.name||d.networkId}<small>{d.role==="primary"?"elsődleges":d.role==="secondary"?"másodlagos":"stacionárius"}</small></span>}):<em>Nincs eszköz hozzárendelve.</em>}</div><div className="personActionsV19"><label className="ghost fileGhostV19">Kép feltöltése<input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>avatar(p,e.target.files?.[0])}/></label><button className="ghost" onClick={()=>startEdit(p)}>Szerkesztés</button><button className="deleteBtn" onClick={()=>remove(p)}>Törlés</button></div>{ps?.note&&<p className="personNoteV19">{ps.note}</p>}</article>})}{!people.length&&<section className="panel empty">Még nincs személy felvéve.</section>}</section>
  {creating&&<section className="panel personEditorV19"><div className="sectionHead"><div><h2>{editing?`${editing.name} szerkesztése`:"Új személy"}</h2><p>Az elsődleges jelenléti eszköz lehetőleg az a telefon legyen, amit az illető magával visz.</p></div><button className="ghost" onClick={reset}>Bezárás</button></div><div className="personFormV19"><label>Név<input value={name} onChange={e=>setName(e.target.value)} placeholder="Név"/></label><label>Becenév<input value={nickname} onChange={e=>setNickname(e.target.value)} placeholder="Opcionális"/></label><label>Szerep<input value={role} onChange={e=>setRole(e.target.value)} placeholder="Családtag"/></label></div><h3>Eszközök hozzárendelése</h3><div className="deviceAssignV19">{assignable.map(n=><article key={n.id}><div><span className={`deviceDot ${n.online?"on":""}`}></span><strong>{n.name}</strong><small>{n.ip||n.mac||"Nincs IP"}</small></div><select value={links[n.id]||""} onChange={e=>setLinks(v=>({...v,[n.id]:e.target.value as PersonDeviceRole|""}))}><option value="">Nincs hozzárendelve</option><option value="primary">Elsődleges jelenlét</option><option value="secondary">Másodlagos személyes</option><option value="stationary">Otthon maradhat (PC/laptop)</option></select></article>)}</div>{assignable.length===0&&<div className="infoLine">Nincs hozzárendelhető kliens a Bridge pillanatképében. Az Archer C6 mögötti telefonokhoz később klienslista-adapter vagy közös LAN/AP mód kell.</div>}<div className="personEditorActionsV19"><button className="ghost" onClick={reset}>Mégse</button><button className="primaryAction" disabled={busy||!name.trim()} onClick={save}>{busy?"Mentés…":"Mentés"}</button></div></section>}
  </div>
}
...
function App(){
  ...
  return <div className="hhApp">{notice&&<div className="toast">{notice}</div>}{deleteTarget&&<DeleteDialog torrent={deleteTarget} onClose={()=>setDeleteTarget(null)} onDelete={removeTorrent}/>} {resolvedDetail&&<DeviceDetailDialog device={resolvedDetail} onClose={()=>setDetailTarget(null)} onCommand={smartCommand}/>} {vacuumDetail&&vacuum&&<VacuumDetailDialog vacuum={vacuum} onClose={()=>setVacuumDetail(false)} onAction={vacuumAction}/>}
    ...
    {tab==="people"&&<PeopleTab people={people} presence={presence} network={network} reload={load} flash={flash}/>}
