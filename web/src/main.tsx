...
function App(){
  ...
  useEffect(()=>{if(auth!=="yes"||!currentUser)return;if(!currentUser.isAdmin&&tab!=="notifications"&&!currentUser.permissions.includes(tab as MenuPermission)){const first=(currentUser.permissions[0]||"overview") as Tab;setTab(first);window.history.replaceState(null,"",`#${first}`)}},[auth,currentUser,tab]);
  ...
  function chooseTab(next:Tab){if(!currentUser)return;if(!currentUser.isAdmin&&next!=="notifications"&&!currentUser.permissions.includes(next as MenuPermission))return;setTab(next);window.history.replaceState(null,"",`#${next}`);window.scrollTo({top:0,behavior:"smooth"})}
  ...
  return <div className="hhApp">{notice&&<div className="toast">{notice}</div>}{deleteTarget&&<DeleteDialog torrent={deleteTarget} onClose={()=>setDeleteTarget(null)} onDelete={removeTorrent}/>} {resolvedDetail&&<DeviceDetailDialog device={resolvedDetail} onClose={()=>setDetailTarget(null)} onCommand={smartCommand}/>} {vacuumDetail&&vacuum&&<VacuumDetailDialog vacuum={vacuum} onClose={()=>setVacuumDetail(false)} onAction={vacuumAction}/>}
    <aside className="hhSidebar">
      ...
      <nav className="hhNav hhDesktopNav" aria-label="HomeHub funkciók">{allowedTabs.map(t=><button key={t.id} className={tab===t.id?"active":""} onClick={()=>chooseTab(t.id)}><span className="hhNavIcon">{t.id==="overview"?"⌂":t.id==="people"?"◉":t.id==="timeline"?"◷":t.id==="downloads"?"↓":t.id==="media"?"▶":t.id==="smart"?"◇":t.id==="actions"?"⚡":t.id==="notifications"?"🔔":t.id==="ai"?"✦":t.id==="network"?"⌁":t.id==="credentials"?"🔑":t.id==="printer"?"▣":"⚙"}</span><span>{t.label}</span>{t.id==="notifications"&&automation.unread>0&&<i className="navAlertBadge">{automation.unread}</i>}</button>)}</nav><nav className="hhMobileNav" aria-label="Gyors navigáció">{mobileTabs.map(t=><button key={t.id} className={tab===t.id?"active":""} onClick={()=>chooseTab(t.id)}><span>{t.id==="overview"?"⌂":t.id==="people"?"◉":t.id==="smart"?"◇":t.id==="actions"?"⚡":t.id==="notifications"?"🔔":"⌁"}</span><small>{t.short}</small>{t.id==="notifications"&&automation.unread>0&&<i className="navAlertBadge">{automation.unread}</i>}</button>)}</nav>
