...
type NetworkSwitchPort = {port:number;label?:string;enabled:boolean;linkUp:boolean;speedMbps:number;duplex:string;configSpeed:string;flowControl:boolean;txPackets?:number;rxPackets?:number;health:string};
type NetworkManagedStatus = {adapter:string;credentialsConfigured:boolean;authOk:boolean;model?:string;hardware?:string;firmware?:string;gateway?:string;ports?:NetworkSwitchPort[];error?:string;updatedAt:string};
type NetworkIdentity = {source:"configured"|"manual"|"known_device"|"tuya_factory"|"tuya_device_id"|"proxy";confidence:number;label:string;owner?:string;deviceType?:string;tuyaDeviceId?:string;tuyaProductName?:string;matchedMac?:string;note?:string};
type NetworkStatus = { id:string; name:string; kind:string; online:boolean; adminOnline?:boolean; ip:string; configuredIp?:string; ipSource?:string; ipChanged?:boolean; mac:string; latencyMs:number; adminUrl:string; note:string; managed?:NetworkManagedStatus; visibility?:string; rawName?:string; identity?:NetworkIdentity };
type NetworkEvent = {id:string;type:"online"|"offline"|"ip_changed"|"link_speed";networkId:string;deviceName:string;message:string;createdAt:string;port?:number;fromValue?:string;toValue?:string};
...
type VaultEntryMeta = {id:string;label:string;kind?:string;username?:string;adminUrl?:string;ip?:string;hasPassword:boolean;saved:boolean;updatedAt?:string};
type VaultStatus = {enabled:boolean;initialized:boolean;pinConfigured:boolean;localUrl:string;entries:VaultEntryMeta[];updatedAt:string;error?:string};
type MenuPermission = "overview"|"people"|"timeline"|"downloads"|"media"|"smart"|"actions"|"ai"|"network"|"credentials"|"printer"|"settings";
type PersonAuthMeta = {enabled:boolean;loginName:string;permissions:MenuPermission[];forcePasswordChange?:boolean;hasPassword?:boolean};
type AuthUser = {id:string;personId?:string;name:string;isAdmin:boolean;permissions:MenuPermission[]};
type PersonDeviceRole = "primary"|"secondary"|"stationary";
type PersonDeviceLink = {networkId:string;role:PersonDeviceRole;label?:string};
type NotificationChannel = "push"|"email"|"sms";
type NotificationPriority = "info"|"normal"|"warning"|"critical";
type PersonNotificationPrefs = {pushEnabled:boolean;emailEnabled:boolean;smsEnabled:boolean};
type PersonProfile = {id:string;name:string;nickname?:string;role?:string;email?:string;phone?:string;hasEmail?:boolean;hasPhone?:boolean;notificationPrefs?:PersonNotificationPrefs;pushSubscriptionCount?:number;auth?:PersonAuthMeta;devices:PersonDeviceLink[];createdAt:string;updatedAt:string;hasAvatar?:boolean};
type PresenceStatus = {personId:string;name:string;status:"home"|"away"|"uncertain";confidence:number;since?:string;lastSeenAt?:string;source?:string;networkId?:string;note?:string};
type ExternalSignal = {key:string;label?:string;category?:"geofence"|"ble"|"generic";value:string|number|boolean;source?:string;personId?:string;updatedAt:string;expiresAt?:string};
type Life360Member = {id:string;firstName?:string;lastName?:string;loginEmail?:string;location?:{latitude?:string;longitude?:string;battery?:string;speed?:string;name?:string;wifiState?:string}};
type Life360Status = {configured:boolean;online:boolean;experimental:true;error?:string;lastUpdatedAt:string|null;circleId?:string;circleName?:string;members:Life360Member[];mapping:Record<string,string>};
type HistoryEvent = {id:string;category:"presence"|"network"|"security"|"smart"|"energy"|"automation"|"system";type:string;entityId?:string;entityName?:string;message:string;createdAt:string;data?:Record<string,unknown>};
type TuyaPoint = { code:string; value:unknown };
type TuyaSpec = { code:string; type:string; values:string; dp_id?:number; dpId?:number };
type TuyaDevice = { id:string; name:string; online:boolean; category:string; productName:string; productId?:string; homeId?:string; mac?:string; uuid?:string; serialNumber?:string; profile?:"mygate"|"feyree"|"aircon"; status:TuyaPoint[]; functions:TuyaSpec[]; statusSpec:TuyaSpec[] };
...
type AutomationTrigger =
  ...
  | {type:"signal.state";key:string;operator:"eq"|"neq";value:string|number|boolean;forSeconds?:number;maxAgeSeconds?:number}
...
type AutomationNotificationPlan = {enabled:boolean;priority:NotificationPriority;recipientPersonIds:string[];channels:NotificationChannel[];fallbackToAdmin?:boolean;escalations?:Array<{afterSeconds:number;channels:NotificationChannel[]}>};
type AutomationRule = {id:string;name:string;enabled:boolean;trigger:AutomationTrigger;actions:AutomationAction[];cooldownSeconds:number;notifyEmail?:boolean;notification?:AutomationNotificationPlan;safety?:{allowGateAction?:boolean};createdAt:string;updatedAt:string;lastTriggeredAt?:string};
type NotificationDelivery = {personId?:string;personName?:string;channel:NotificationChannel;target?:string;ok:boolean;skipped?:boolean;error?:string};
type AlertRecord = {id:string;ruleId:string;ruleName:string;subject:string;message:string;createdAt:string;priority?:NotificationPriority;emailRequested:boolean;emailSent:boolean;emailError?:string;deliveries?:NotificationDelivery[];escalationLevel?:number;readAt?:string};
type AutomationState = {rules:AutomationRule[];alerts:AlertRecord[];unread:number;email:{configured:boolean;recipients:number};notification:{emailConfigured:boolean;fallbackEmailRecipients:number;pushConfigured:boolean;smsConfigured:boolean}};
...
type AIAutomationDraftResult = {explanation:string;draft:null|{name:string;enabled:boolean;trigger:AutomationTrigger;actions:AutomationAction[];cooldownSeconds:number;notifyEmail?:boolean;notification?:AutomationNotificationPlan;safety?:{allowGateAction?:boolean}};valid:boolean;warnings:string[];usage?:AIUsage};
...
type State = {
  ...
  ai:AIState;
  persistence?:{bootstrapPending:boolean};
  life360?:Life360Status;
};
...
const tabDefs:Array<{id:Tab;label:string;short:string}>=[
  ...
  {id:"people",label:"Emberek",short:"Emberek"},
  {id:"timeline",label:"Idővonal",short:"Idővonal"},
  ...
  {id:"credentials",label:"Hozzáférések",short:"Hozzáférés"},
...
function apiErrorText(body:any,status:number){
  const err=body?.error;
  if(typeof err==="string"){
    const map:Record<string,string>={password_required_for_first_enable:"Az első bekapcsoláshoz legalább 8 karakteres jelszó szükséges.",login_name_required:"Adj meg legalább 2 karakteres felhasználónevet.",reserved_login_name:"Az admin felhasználónév fenntartott.",login_name_in_use:"Ezt a felhasználónevet már használja másik személy.",person_not_found:"A személy profilja nem található.",forbidden:"Ehhez nincs jogosultságod."};
    return map[err]||err;
  }
  if(err&&typeof err==="object"){
    const fields=err.fieldErrors&&typeof err.fieldErrors==="object"?Object.entries(err.fieldErrors).flatMap(([k,v])=>(Array.isArray(v)?v:[]).map(x=>`${k}: ${x}`)):[];
    const forms=Array.isArray(err.formErrors)?err.formErrors:[];
    const parts=[...forms,...fields].filter(Boolean);
    if(parts.length)return parts.join(" · ");
  }
  return body?.message||`HTTP ${status}`;
}
async function api(path:string,init?:RequestInit){const r=await fetch(path,init);if(r.status===401)throw new Error("AUTH_REQUIRED");const b=await r.json().catch(()=>({}));if(r.status===403)throw new Error("FORBIDDEN");if(!r.ok)throw new Error(apiErrorText(b,r.status));return b}
...
function Login({onDone}:{onDone:(user:AuthUser)=>void}){
  ...
  async function submit(e:React.FormEvent){e.preventDefault();setBusy(true);setError("");try{const r=await api("/api/auth/login",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({login,password})}) as {user:AuthUser};onDone(r.user)}catch(err){setError(err instanceof Error&&err.message==="too_many_attempts"?"Túl sok sikertelen próbálkozás. Próbáld később.":"Hibás felhasználónév vagy jelszó.")}finally{setBusy(false)}}
  return <main className="loginShell"><section className="loginCard"><div className="brandMark">⌂</div><div className="eyebrow">HOME HUB</div><h1>Belépés</h1><p>Személyes HomeHub hozzáférés. Admin belépéshez a felhasználónév mező maradhat üres.</p><form onSubmit={submit}><input type="text" autoFocus autoComplete="username" value={login} onChange={e=>setLogin(e.target.value)} placeholder="Felhasználónév / admin"/><input type="password" autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Jelszó"/><button disabled={busy||!password}>{busy?"Belépés…":"Belépés"}</button></form>{error&&<div className="loginError">{error}</div>}</section></main>
}
...
function DeviceDetailDialog({device,onClose,onCommand}:{device:TuyaDevice;onClose:()=>void;onCommand:(d:TuyaDevice,code:string,value:unknown)=>void}){
  ...
  return <div className="modalBack" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><section className={`modal deviceDetail ${detailClass}`}><div className="detailHead"><div><span className="deviceKind">{labelKind(kind)}</span><h2>{device.name}</h2><p>{friendlyProductName(device)} · {device.online?"Online":"Offline"}{device.mac?` · MAC ${device.mac}`:""}</p></div><button className="closeBtn" onClick={onClose}>×</button></div>
...
function NetworkDeviceCard({n,onIdentify}:{n:NetworkStatus;onIdentify:(n:NetworkStatus)=>void}){
  const indirect=n.visibility==="indirect",id=n.identity;
  const sourceLabel=id?.source==="tuya_factory"?"Tuya MAC":id?.source==="tuya_device_id"?"Tuya ID":id?.source==="known_device"?"Megerősített":id?.source==="manual"?"Kézi":id?.source==="proxy"?"Mesh proxy":"";
  return <article className={`networkDevice ${indirect?"indirect":n.online?"online":"offline"} ${id?"identifiedV20":""}`}><div className="deviceHead"><div><span className={`deviceDot ${n.online?"on":indirect?"indirect":""}`}></span><strong>{n.name}</strong></div><span className="deviceKind">{id?.deviceType||networkKind(n.kind)}</span></div><small>{n.ip||"IP keresése…"}{n.mac?` · ${n.mac}`:""}</small>{id&&<div className={`identityBadgeV20 ${id.source}`}><b>{sourceLabel}</b><span>{id.confidence}%</span>{id.owner&&<span>{id.owner}</span>}{id.tuyaDeviceId&&<span>Tuya · {id.tuyaDeviceId.slice(0,8)}…</span>}</div>}{n.ipChanged&&<div className="ipChangeBadge">DHCP/IP változás · konfigurált: {n.configuredIp}</div>}<div className="networkMeta"><span>{indirect?"Archer LAN mögött":n.online?(n.latencyMs>0?`${n.latencyMs.toFixed(1)} ms`:"Jelen van"):"Nem elérhető"}</span>{n.ipSource==="arp-mac"&&<span>MAC → IP</span>}{n.adminUrl&&<a className={n.adminOnline===false&&!indirect?"mutedLink":""} href={n.adminUrl} target="_blank" rel="noreferrer">Admin{n.adminOnline===false&&!indirect?" (nem válaszol)":""}</a>}</div><em>{id?.note||n.note}</em>{n.mac&&<div className="networkIdentifyActionsV20"><button className="ghost" onClick={()=>onIdentify(n)}>{id?.source==="manual"?"Azonosítás szerkesztése":"Azonosítás"}</button></div>}{n.managed&&<small className={n.managed.authOk?"managedMini ok":"managedMini"}>{n.managed.authOk?`${n.managed.ports?.filter(p=>p.linkUp).length||0} aktív switch port`:n.managed.error||"Részletes integráció nincs kész"}</small>}</article>
}
...
function ManagedSwitchPanel({network}:{network:NetworkStatus[]}){
  const managed=network.filter(n=>n.managed?.adapter==="tplink-easy-smart");
  if(!managed.length)return null;
  return <section className="managedNetworkBlock"><div className="sectionHead"><div><h3>Menedzselhető switchek</h3><p>Élő portsebesség és portstátusz a helyi adminfelületről. A jelszó kizárólag a WD Bridge-en marad.</p></div></div>{managed.map(sw=>{
    const m=sw.managed!;const slow=(m.ports||[]).filter(p=>p.linkUp&&p.speedMbps>0&&p.speedMbps<1000);
    return <article className="switchMonitor" key={sw.id}><div className="switchMonitorHead"><div><strong>{sw.name}</strong><small>{m.hardware||m.model||sw.ip}{m.firmware?` · ${m.firmware}`:""}</small></div><span className={m.authOk?"integrationBadge ok":"integrationBadge warn"}>{m.authOk?"Admin kapcsolat OK":m.credentialsConfigured?"Belépési hiba":"Jelszó szükséges"}</span></div>{m.error&&<div className="networkWarning">{m.error}</div>}{m.authOk&&<><div className="switchPortGrid">{(m.ports||[]).map(port=><div className={`switchPort ${port.health}`} key={port.port}><div><b>Port {port.port}</b>{port.label&&<small>{port.label}</small>}</div><strong>{port.linkUp?`${port.speedMbps} Mbps`:"Link Down"}</strong><span>{port.linkUp?(port.duplex==="full"?"Full duplex":port.duplex):port.enabled?"Nincs kapcsolat":"Letiltva"}</span></div>)}</div>{slow.length>0&&<div className="networkWarning strong">⚠ {slow.map(p=>`Port ${p.port}${p.label?` (${p.label})`:""}: ${p.speedMbps} Mbps`).join(" · ")}</div>}</>}</article>
  })}</section>
}

function NetworkIntegrationStatus({network}:{network:NetworkStatus[]}){
  const infra=network.filter(n=>["gateway","router","extender","switch","nas"].includes(n.kind));
  return <section className="managedNetworkBlock"><div className="sectionHead"><div><h3>Hálózati infrastruktúra</h3><p>MAC-alapú újrafelderítés védi a Home Hubot a DHCP/IP-változásoktól. A routerek és erősítők részletes helyi adaptere eszköz- és firmware-függő.</p></div></div><div className="integrationGrid">{infra.map(n=><article key={n.id} className="integrationCard"><div><span className={`deviceDot ${n.online?"on":""}`}></span><strong>{n.name}</strong></div><small>{n.ip||"IP keresése"}{n.mac?` · ${n.mac}`:""}</small><div className="integrationMeta"><span>{n.ipSource==="arp-mac"?"MAC alapján felderítve":"Konfigurált IP"}</span>{n.ipChanged&&<b>IP változott</b>}</div><p>{n.managed?.authOk?"Részletes helyi adminadatok aktívak.":n.kind==="switch"&&n.managed?"A részletes switch-adatokhoz add meg a helyi admin jelszót a WD-n.":"Jelenleg élő ping/ARP/admin-port monitorozás. Részletes adapterhez helyi admin-hozzáférés kell, TP-Link Cloud nem szükséges."}</p></article>)}</div></section>
}

function NetworkEventTimeline({events}:{events:NetworkEvent[]}){
  return <section className="managedNetworkBlock"><div className="sectionHead"><div><h3>Hálózati események</h3><p>Újraindulás, IP-váltás és switch-port sebességváltozás naplója.</p></div><span className="sectionCounter">{events.length}</span></div>{events.length===0?<div className="empty">Még nincs rögzített hálózati változás.</div>:<div className="networkEventList">{events.slice(0,30).map(e=><article key={e.id}><span className={`eventType ${e.type}`}></span><div><strong>{e.message}</strong><small>{new Date(e.createdAt).toLocaleString("hu-HU")}</small></div></article>)}</div>}</section>
}
...
function NetworkTopology({network}:{network:NetworkStatus[]}){
  ...
  return <div className="topologyWrap">
    ...
    <div className="topology topologyV11">
      <div className="topoRoot"><TopoNode title="Technicolor FGA2233" subtitle="Fő router" status={byId["technicolor-fga2233"]} kind="gateway"/></div>
      <div className="topologySections">
        <section className="topoLane wifiLane"><header><span className="laneBadge wifi">Wi-Fi</span><strong>krankovics2</strong></header><div className="wifiDirect"><TopoNode title="Krankovics-MBP" subtitle="2.4 GHz" status={byId["krankovics-mbp"]} kind="computer"/></div></section>
        <section className="topoLane ethernet ethernetV11"><header><span className="laneBadge ethernet">Ethernet</span><strong>Vezetékes hálózat</strong></header><div className="portsV11">
          <div className="portV11"><div className="portLabel">Port 1</div><TopoNode title="DESKTOP-E6K3SEK" subtitle="Vezetékes gép" status={byId["desktop-e6k3sek"]} kind="computer"/></div>
          <div className="portV11 port2"><div className="portLabel">Port 2</div><TopoNode title="TL-SG108E" subtitle="Menedzselhető switch" status={byId["tl-sg108e"]} kind="network-switch"/><div className="port2Tree"><div className="treeBranch"><TopoNode title="DorkaPC" subtitle="Vezetékes gép" status={byId["dorkapc"]} kind="computer"/></div><div className="treeBranch chainBranch"><TopoNode title="D-Link GO-SW-5G" subtitle="Passzív gigabit switch" kind="network-switch"/><span className="chainArrow">↓</span><TopoNode title="TP-Link LiteWave LS105G" subtitle="Passzív gigabit switch" kind="network-switch"/><span className="chainArrow">↓</span><TopoNode title="davidgaming" subtitle="Vezetékes gép" status={byId["davidgaming"]} kind="computer"/></div><div className="treeBranch meshBranchV11"><TopoNode title="Archer C6" subtitle="Mesh főpont" status={byId["archer-c6"]} kind="router"/><div className="ssidLabel">Wi-Fi / mesh: <strong>krankovics</strong></div><div className="meshNodesV11"><TopoNode title="RE220" status={byId["re220"]} kind="extender"/><TopoNode title="RE315 #1" subtitle="MAC …:93:86" status={byId["re315-1"]} kind="extender"/><TopoNode title="RE315 #2" status={byId["re315-2"]} kind="extender"/></div></div></div></div>
          <div className="portV11"><div className="portLabel">Port 3</div><TopoNode title="KD20 / oldnas" subtitle="Torrent NAS" status={byId["kd20"]} kind="nas"/></div>
          <div className="portV11"><div className="portLabel">Port 4</div><TopoNode title="WD My Cloud" subtitle="Médiatár" status={byId["wd-my-cloud"]} kind="nas"/></div>
        </div></section>
      </div>
    </div>
  </div>
...
function triggerLabel(rule:AutomationRule,smart:SmartHome,network:NetworkStatus[],people:PersonProfile[]=[],signals:ExternalSignal[]=[]){
  const render=(t:AutomationTrigger):string=>{
    ...
    if(t.type==="network.online"||t.type==="network.offline"){const n=network.find(x=>x.id===t.networkId);return `${n?.name||"Hálózati eszköz"} ${t.type==="network.online"?"online":"offline"}${t.forSeconds?` · ${Math.round(t.forSeconds/60)} percig`:""}`}
    if(t.type==="network.link_below"){const n=network.find(x=>x.id===t.networkId);return `${n?.name||"Switch"} · Port ${t.port} < ${t.mbps} Mbps${t.forSeconds?` · ${Math.round(t.forSeconds/60)} percig`:""}`}
    if(t.type==="network.new_device")return"Új MAC-cím jelenik meg a helyi hálózaton";
    if(t.type==="presence.person_state"){const p=people.find(x=>x.id===t.personId);return `${p?.name||"Személy"} · ${t.state==="home"?"itthon":t.state==="away"?"nincs itthon":"bizonytalan"}${t.forSeconds?` · ${Math.round(t.forSeconds/60)} percig`:""}`}
    if(t.type==="presence.device_mismatch"){const p=people.find(x=>x.id===t.personId);return `${p?.name||"Személy"}: elsődleges eszköz offline, másodlagos még online${t.forSeconds?` · ${Math.round(t.forSeconds/60)} percig`:""}`}
    if(t.type==="signal.state"||t.type==="signal.numeric"){const sig=signals.find(x=>x.key===t.key);const opMap:Record<"gt"|"gte"|"lt"|"lte"|"eq"|"neq",string>={gt:">",gte:"≥",lt:"<",lte:"≤",eq:"=",neq:"≠"};const op=opMap[t.operator];return `${sig?.label||t.key} ${op} ${String(t.value)}${t.forSeconds?` · ${Math.round(t.forSeconds/60)} percig`:""}`}
    if(t.type==="all")return t.conditions.map(render).join(" + ")+(t.forSeconds?` · együtt ${Math.round(t.forSeconds/60)} percig`:"");
    return `Ütemezés · ${t.time} · ${t.days.length===7?"minden nap":t.days.map(x=>["V","H","K","Sze","Cs","P","Szo"][x]).join(", ")}`;
  };
  return render(rule.trigger);
}

function actionLabel(action:AutomationAction,smart:SmartHome){
  if(action.type==="alert")return "HomeHub értesítés";
...
function parseAutomationValue(raw:string,current?:unknown):string|number|boolean{const v=raw.trim();if(typeof current==="boolean"||/^(true|false|igen|nem|be|ki)$/i.test(v))return /^(true|igen|be)$/i.test(v);if(typeof current==="number"&&v!==""&&!Number.isNaN(Number(v)))return Number(v);return v}
function automationPointValue(device?:TuyaDevice,point?:TuyaPoint):unknown{if(!point||typeof point.value!=="number"||!Number.isFinite(point.value))return point?.value;const spec=[...(device?.functions||[]),...(device?.statusSpec||[])].find(x=>x.code===point.code);let scale=0;try{const meta=JSON.parse(spec?.values||"{}");scale=Number(meta?.scale||0);if(!Number.isFinite(scale))scale=0}catch{}let value=Number(point.value)/Math.pow(10,scale);if(scale===0&&/va_temperature/i.test(point.code)&&Math.abs(value)>100)value/=10;return value}

type ActionTemplate="temp_climate"|"gate_alert"|"presence_mismatch"|"generic_sensor"|"external_signal"|"arrival_gate"|"departure_gate"|"machine_night"|"network_offline"|"slow_link"|"vacuum_schedule"|"new_device"|"ai_summary";
function AutomationComposer({smart,network,people,signals,vacuum,onClose,onSaved,flash}:{smart:SmartHome;network:NetworkStatus[];people:PersonProfile[];signals:ExternalSignal[];vacuum?:VacuumStatus;onClose:()=>void;onSaved:()=>void;flash:(m:string)=>void}){
  const sensors=smart.devices.filter(d=>deviceKind(d)==="sensor"),climates=smart.devices.filter(d=>deviceKind(d)==="climate"),gates=smart.devices.filter(d=>deviceKind(d)==="gate"),chargers=smart.devices.filter(d=>deviceKind(d)==="charger"),machines=network.filter(n=>["computer","discovered"].includes(n.kind)),infra=network.filter(n=>["gateway","router","extender","switch","nas"].includes(n.kind)),managedSwitches=network.filter(n=>(n.managed?.ports||[]).length>0),statusDevices=smart.devices.filter(d=>d.status?.length);
  ...
  const[infraId,setInfraId]=useState(infra[0]?.id||"");const[offlineMinutes,setOfflineMinutes]=useState("5");const[switchId,setSwitchId]=useState(managedSwitches[0]?.id||"");const[linkPort,setLinkPort]=useState("1");const[linkThreshold,setLinkThreshold]=useState("1000");const[linkHold,setLinkHold]=useState("2");
  const[vacTime,setVacTime]=useState("10:00");const[days,setDays]=useState<number[]>([1,2,3,4,5]);const[summaryTime,setSummaryTime]=useState("20:30");const[mismatchPersonId,setMismatchPersonId]=useState(people[0]?.id||"");const[mismatchMinutes,setMismatchMinutes]=useState("5");const[notifyEnabled,setNotifyEnabled]=useState(true);const[recipientIds,setRecipientIds]=useState<string[]>(people[0]?.id?[people[0].id]:[]);const[notifyChannels,setNotifyChannels]=useState<NotificationChannel[]>(["push","email"]);const[priority,setPriority]=useState<NotificationPriority>("warning");const[escalate,setEscalate]=useState(false);const[escalationMinutes,setEscalationMinutes]=useState("10");const[escalationChannels,setEscalationChannels]=useState<NotificationChannel[]>(["sms"]);const[busy,setBusy]=useState(false);
  const[genericDeviceId,setGenericDeviceId]=useState(statusDevices[0]?.id||"");const[genericCode,setGenericCode]=useState(statusDevices[0]?.status?.[0]?.code||"");const[genericOperator,setGenericOperator]=useState<"gt"|"gte"|"lt"|"lte"|"eq"|"neq">("eq");const[genericExpected,setGenericExpected]=useState(String(automationPointValue(statusDevices[0],statusDevices[0]?.status?.[0])??""));const[genericHold,setGenericHold]=useState("0");
  const[signalKey,setSignalKey]=useState(signals[0]?.key||"");const[signalExpected,setSignalExpected]=useState(String(signals[0]?.value??"home"));const[signalHold,setSignalHold]=useState("0");
  const[arrivalPersonId,setArrivalPersonId]=useState(people[0]?.id||"");const[arrivalSignalKey,setArrivalSignalKey]=useState(signals.find(x=>x.category==="geofence"||x.category==="ble")?.key||"");const[arrivalSignalExpected,setArrivalSignalExpected]=useState("home");const[arrivalGateId,setArrivalGateId]=useState(gates[0]?.id||"");const[arrivalHold,setArrivalHold]=useState("0");
  const[departureNetworkId,setDepartureNetworkId]=useState(machines[0]?.id||"");const[departureChargerId,setDepartureChargerId]=useState(chargers[0]?.id||"");const[departureCode,setDepartureCode]=useState(chargers[0]?.status?.[0]?.code||"");const[departureExpected,setDepartureExpected]=useState(String(chargers[0]?.status?.[0]?.value??"false"));const[departureGateId,setDepartureGateId]=useState(gates[0]?.id||"");const[departureHold,setDepartureHold]=useState("1");const[gateSafetyConfirmed,setGateSafetyConfirmed]=useState(false);
  ...
  function gateCommand(deviceId:string,kind:"open"|"close"){const d=gates.find(x=>x.id===deviceId);if(!d)return null;return actionFunction(d,kind==="open"?["open_1","gate_open","door_open","open","start_1","start"]:["close_1","gate_close","door_close","close","start_1","start"])}
  function genericDevice(){return statusDevices.find(x=>x.id===genericDeviceId)}
  function genericPoint(){return genericDevice()?.status.find(x=>x.code===genericCode)}
  function genericValue(){return automationPointValue(genericDevice(),genericPoint())}
  function toggleRecipient(id:string){setRecipientIds(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id])}
  function toggleChannel(id:NotificationChannel,escalation=false){const setter=escalation?setEscalationChannels:setNotifyChannels;setter(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id])}
  async function save(){
    let payload:any;
    ...
    }else if(template==="presence_mismatch"){
      const person=people.find(x=>x.id===mismatchPersonId);if(!person)return flash("Válassz személyt.");
      payload={name:`${person.name}: telefon eltűnt, másodlagos eszköz itthon`,enabled:true,trigger:{type:"presence.device_mismatch",personId:person.id,forSeconds:Math.max(1,Number(mismatchMinutes))*60},actions:[{type:"alert",subject:`HomeHub: ${person.name} jelenléti eszközei eltérnek`,message:"{{detail}}\nA HomeHub emiatt nem tekinti biztos távozásnak.",email:true}],cooldownSeconds:900};
    }else if(template==="generic_sensor"){
      const d=genericDevice(),point=genericPoint();if(!d||!point)return flash("Válassz eszközt és állapotmezőt.");const current=automationPointValue(d,point),expected=parseAutomationValue(genericExpected,current),hold=Math.max(0,Number(genericHold))*60;
      if(typeof current==="number"){if(genericOperator==="neq")return flash("Numerikus szenzornál válassz =, >, ≥, < vagy ≤ feltételt.");const numericExpected=Number(expected);if(!Number.isFinite(numericExpected))return flash("A kiválasztott numerikus mezőhöz számérték szükséges.");payload={name:`${d.name}: ${genericCode} ${genericOperator} ${numericExpected}`,enabled:true,trigger:{type:"tuya.numeric",deviceId:d.id,code:genericCode,operator:genericOperator as "gt"|"gte"|"lt"|"lte"|"eq",value:numericExpected,forSeconds:hold},actions:[{type:"alert",subject:`HomeHub: ${d.name}`,message:"{{detail}}\nÉszlelés: {{time}}",email:true}],cooldownSeconds:300};}
      else payload={name:`${d.name}: ${genericCode} ${genericOperator} ${String(expected)}`,enabled:true,trigger:{type:"tuya.state",deviceId:d.id,code:genericCode,operator:genericOperator==="neq"?"neq":"eq",value:expected,forSeconds:hold},actions:[{type:"alert",subject:`HomeHub: ${d.name}`,message:"{{detail}}\nÉszlelés: {{time}}",email:true}],cooldownSeconds:300};
    }else if(template==="external_signal"){
      const key=signalKey.trim();if(!key)return flash("Adj meg külső jel kulcsot.");const current=signals.find(x=>x.key===key)?.value;payload={name:`Külső jel: ${key} = ${signalExpected}`,enabled:true,trigger:{type:"signal.state",key,operator:"eq",value:parseAutomationValue(signalExpected,current),forSeconds:Math.max(0,Number(signalHold))*60,maxAgeSeconds:900},actions:[{type:"alert",subject:`HomeHub: ${key}`,message:"{{detail}}\nÉszlelés: {{time}}",email:true}],cooldownSeconds:300};
    }else if(template==="arrival_gate"){
      const person=people.find(x=>x.id===arrivalPersonId),gate=gates.find(x=>x.id===arrivalGateId),fn=gateCommand(arrivalGateId,"open");if(!person||!gate||!fn)return flash("Válassz személyt, kaput és elérhető nyitási parancsot.");if(!arrivalSignalKey.trim())return flash("Adj meg geofence/BLE jel kulcsot.");if(!gateSafetyConfirmed)return flash("A kapu automatizálásához jelöld be a biztonsági megerősítést.");payload={name:`${person.name} érkezik → ${gate.name} nyit`,enabled:false,trigger:{type:"all",conditions:[{type:"presence.person_state",personId:person.id,state:"home",forSeconds:0},{type:"signal.state",key:arrivalSignalKey.trim(),operator:"eq",value:parseAutomationValue(arrivalSignalExpected,signals.find(x=>x.key===arrivalSignalKey.trim())?.value),forSeconds:0,maxAgeSeconds:300}],forSeconds:Math.max(0,Number(arrivalHold))*60},actions:[{type:"tuya.command",deviceId:gate.id,code:fn.code,value:true}],cooldownSeconds:600,safety:{allowGateAction:true}};
    }else if(template==="departure_gate"){
      const car=network.find(x=>x.id===departureNetworkId),charger=chargers.find(x=>x.id===departureChargerId),point=charger?.status.find(x=>x.code===departureCode),gate=gates.find(x=>x.id===departureGateId),fn=gateCommand(departureGateId,"close");if(!car||!charger||!point||!gate||!fn)return flash("Válassz autó/Wi-Fi eszközt, töltőállapotot és kaput.");if(!gateSafetyConfirmed)return flash("A kapu automatizálásához jelöld be a biztonsági megerősítést.");payload={name:`${car.name} elment + töltő feltétel → ${gate.name} zár`,enabled:false,trigger:{type:"all",conditions:[{type:"network.offline",networkId:car.id,forSeconds:0},{type:"tuya.state",deviceId:charger.id,code:departureCode,operator:"eq",value:parseAutomationValue(departureExpected,point.value),forSeconds:0}],forSeconds:Math.max(0,Number(departureHold))*60},actions:[{type:"tuya.command",deviceId:gate.id,code:fn.code,value:true}],cooldownSeconds:600,safety:{allowGateAction:true}};
    ...
    }else if(template==="network_offline"){
      if(!infraId)return flash("Válassz hálózati eszközt.");const n=infra.find(x=>x.id===infraId);
      payload={name:`${n?.name||"Hálózati eszköz"} offline`,enabled:true,trigger:{type:"network.offline",networkId:infraId,forSeconds:Math.max(1,Number(offlineMinutes))*60},actions:[{type:"alert",subject:`HomeHub: ${n?.name||"hálózati eszköz"} nem elérhető`,message:"{{detail}}\nA hiba {{time}} időpontban legalább a beállított ideje fennáll.",email:true}],cooldownSeconds:1800};
    }else if(template==="slow_link"){
      if(!switchId)return flash("Nincs részletes portadatot adó switch.");const sw=managedSwitches.find(x=>x.id===switchId);const port=Math.max(1,Number(linkPort)||1);
      payload={name:`${sw?.name||"Switch"} Port ${port} lassú link`,enabled:true,trigger:{type:"network.link_below",networkId:switchId,port,mbps:Math.max(10,Number(linkThreshold)||1000),forSeconds:Math.max(0,Number(linkHold))*60},actions:[{type:"alert",subject:"HomeHub: hálózati link lassult",message:"{{detail}}\nÉszlelés: {{time}}",email:true}],cooldownSeconds:1800};
    ...
    const effectiveRecipientIds = template==="presence_mismatch" && recipientIds.length===0 && mismatchPersonId ? [mismatchPersonId] : template==="arrival_gate" && recipientIds.length===0 && arrivalPersonId ? [arrivalPersonId] : recipientIds;
    if(notifyEnabled&&effectiveRecipientIds.length===0)return flash("Válassz legalább egy értesítési címzettet.");
    if(notifyEnabled&&notifyChannels.length===0)return flash("Válassz legalább egy értesítési csatornát.");
    payload.notifyEmail=false;
    payload.notification={enabled:notifyEnabled,priority,recipientPersonIds:effectiveRecipientIds,channels:notifyChannels,fallbackToAdmin:false,escalations:escalate&&escalationChannels.length?[{afterSeconds:Math.max(1,Number(escalationMinutes))*60,channels:escalationChannels}]:[]};
    const gateAutomation=Boolean(payload.safety?.allowGateAction);
    setBusy(true);try{await api("/api/automations",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});flash(gateAutomation?"Kapu automatizálás létrehozva, biztonsági okból kikapcsolva. Ellenőrzés után kapcsold be.":"Akció létrehozva.");onSaved();onClose()}catch(err){flash(`Akció hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}finally{setBusy(false)}
  }
  const templateDefs:Array<{id:ActionTemplate;title:string;desc:string;icon:string}>=[
    ...
    {id:"gate_alert",title:"Nyitva maradt kapu",desc:"Tartós nyitás után több címzett és több csatorna.",icon:"⌂"},
    {id:"presence_mismatch",title:"Telefon eltűnt, óra maradt",desc:"Elsődleges jelenléti eszköz offline, másodlagos továbbra is online.",icon:"◎"},
    {id:"generic_sensor",title:"Bármely szenzor / állapot",desc:"Víz, mozgás, páratartalom, fogyasztás vagy bármely Tuya DP alapján.",icon:"◇"},
    {id:"external_signal",title:"Geofence / BLE / külső jel",desc:"Telefonos vagy ESP32 jelből készít feltételt. Alapból nem aktív.",icon:"⌖"},
    {id:"arrival_gate",title:"Érkezés → kapunyitás",desc:"Wi-Fi jelenlét + geofence/BLE két jel együtt. Explicit safety engedéllyel.",icon:"↪"},
    {id:"departure_gate",title:"Távozás → kapuzárás",desc:"Wi-Fi offline + töltőállapot együtt. Explicit safety engedéllyel.",icon:"↩"},
    {id:"machine_night",title:"Gép online este",desc:"Időablakban figyeli egy felvett gép jelenlétét.",icon:"◈"},
    {id:"network_offline",title:"Hálózati eszköz offline",desc:"Csak tartós kiesés után küld riasztást.",icon:"!"},
    {id:"slow_link",title:"Switch port belassul",desc:"Például 1 Gbit helyett 100/10 Mbit linkre riaszt.",icon:"↘"},
  ...
  return <div className="modalBack actionModalBack" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><section className="actionComposer"><div className="actionComposerHead"><div><span className="smartEyebrowV12">ÚJ AKCIÓ</span><h2>Automatizálás létrehozása</h2><p>Válassz egy kész mintát, majd állítsd be a feltételeket.</p></div><button className="detailClose" onClick={onClose}>×</button></div><div className="actionTemplateGrid">{templateDefs.map(x=><button key={x.id} className={template===x.id?"active":""} onClick={()=>{setTemplate(x.id);if(x.id==="presence_mismatch"&&mismatchPersonId)setRecipientIds([mismatchPersonId]);if(x.id==="arrival_gate"&&arrivalPersonId)setRecipientIds([arrivalPersonId])}}><i>{x.icon}</i><strong>{x.title}</strong><small>{x.desc}</small></button>)}</div><div className="actionFields">
    ...
    {template==="presence_mismatch"&&<><label>Személy<select value={mismatchPersonId} onChange={e=>{setMismatchPersonId(e.target.value);setRecipientIds([e.target.value])}}>{people.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label>Legalább<input type="number" min="1" value={mismatchMinutes} onChange={e=>setMismatchMinutes(e.target.value)}/><span>percig</span></label><div className="actionInfoBox"><strong>Nem jelent automatikus távozást.</strong><span>Csak akkor aktiválódik, ha az elsődleges eszköz offline, miközben legalább egy másodlagos személyes eszköz még online.</span></div></>}
    {template==="generic_sensor"&&<><label>Tuya eszköz<select value={genericDeviceId} onChange={e=>{const id=e.target.value;setGenericDeviceId(id);const d=statusDevices.find(x=>x.id===id),p=d?.status?.[0];setGenericCode(p?.code||"");setGenericExpected(String(automationPointValue(d,p)??""));if(typeof automationPointValue(d,p)==="number"&&genericOperator==="neq")setGenericOperator("eq")}}>{statusDevices.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select></label><label>Állapotmező<select value={genericCode} onChange={e=>{setGenericCode(e.target.value);const d=statusDevices.find(x=>x.id===genericDeviceId),p=d?.status.find(x=>x.code===e.target.value),v=automationPointValue(d,p);setGenericExpected(String(v??""));if(typeof v==="number"&&genericOperator==="neq")setGenericOperator("eq")}}>{genericDevice()?.status.map(p=><option key={p.code} value={p.code}>{p.code} · most: {String(automationPointValue(genericDevice(),p))}</option>)}</select></label><label>Feltétel<select value={genericOperator} onChange={e=>setGenericOperator(e.target.value as any)}><option value="eq">=</option>{typeof genericValue()!=="number"&&<option value="neq">≠</option>}{typeof genericValue()==="number"&&<><option value="gt">&gt;</option><option value="gte">≥</option><option value="lt">&lt;</option><option value="lte">≤</option></>}</select></label><label>Érték<input value={genericExpected} onChange={e=>setGenericExpected(e.target.value)}/></label><label>Legalább<input type="number" min="0" value={genericHold} onChange={e=>setGenericHold(e.target.value)}/><span>percig</span></label></>}
    {template==="external_signal"&&<><label>Jel kulcsa<input list="signal-keys-v22" value={signalKey} onChange={e=>setSignalKey(e.target.value)} placeholder="geofence.bela"/><datalist id="signal-keys-v22">{signals.map(x=><option key={x.key} value={x.key}>{x.label||x.key}</option>)}</datalist></label><label>Elvárt érték<input value={signalExpected} onChange={e=>setSignalExpected(e.target.value)} placeholder="home"/></label><label>Legalább<input type="number" min="0" value={signalHold} onChange={e=>setSignalHold(e.target.value)}/><span>percig</span></label><div className="actionInfoBox"><strong>A HomeHub nem indít geofence-et magától.</strong><span>A jel natív mobilappból, ESP32-ből vagy hitelesített webhookból érkezhet. Addig a szabály nem teljesül.</span></div></>}
    {template==="arrival_gate"&&<><label>Személy<select value={arrivalPersonId} onChange={e=>{setArrivalPersonId(e.target.value);if(!recipientIds.length)setRecipientIds([e.target.value])}}>{people.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label>Geofence / BLE jel<input list="arrival-signal-keys-v22" value={arrivalSignalKey} onChange={e=>setArrivalSignalKey(e.target.value)} placeholder="geofence.bela"/><datalist id="arrival-signal-keys-v22">{signals.map(x=><option key={x.key} value={x.key}>{x.label||x.key}</option>)}</datalist></label><label>Jel értéke<input value={arrivalSignalExpected} onChange={e=>setArrivalSignalExpected(e.target.value)} placeholder="home"/></label><label>Kapu<select value={arrivalGateId} onChange={e=>setArrivalGateId(e.target.value)}>{gates.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select></label><label>Két jel együtt<input type="number" min="0" value={arrivalHold} onChange={e=>setArrivalHold(e.target.value)}/><span>percig</span></label><div className="actionInfoBox danger"><strong>Két jel szükséges.</strong><span>A kapunyitás csak akkor fut, ha a személy HomeHub-jelenléte itthon és a geofence/BLE jel is megfelel. Router reconnect önmagában nem elég.</span></div><label className="gateSafetyConfirmV22"><input type="checkbox" checked={gateSafetyConfirmed} onChange={e=>setGateSafetyConfirmed(e.target.checked)}/><span><b>Engedélyezem az automatikus fizikai kapumozgatást</b><small>Minden futás naplózódik, a szabály bármikor kikapcsolható.</small></span></label></>}
    {template==="departure_gate"&&<><label>Autó / figyelt Wi-Fi eszköz<select value={departureNetworkId} onChange={e=>setDepartureNetworkId(e.target.value)}>{network.filter(n=>n.kind!=="gateway"&&n.kind!=="switch").map(n=><option key={n.id} value={n.id}>{n.name} · {n.ip||n.mac}</option>)}</select></label><label>Töltő<select value={departureChargerId} onChange={e=>{setDepartureChargerId(e.target.value);const d=chargers.find(x=>x.id===e.target.value);setDepartureCode(d?.status?.[0]?.code||"");setDepartureExpected(String(d?.status?.[0]?.value??""))}}>{chargers.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select></label><label>Töltő állapot<select value={departureCode} onChange={e=>{setDepartureCode(e.target.value);const d=chargers.find(x=>x.id===departureChargerId),p=d?.status.find(x=>x.code===e.target.value);setDepartureExpected(String(p?.value??""))}}>{chargers.find(x=>x.id===departureChargerId)?.status.map(p=><option key={p.code} value={p.code}>{p.code} · most: {String(p.value)}</option>)}</select></label><label>Elvárt érték<input value={departureExpected} onChange={e=>setDepartureExpected(e.target.value)}/></label><label>Kapu<select value={departureGateId} onChange={e=>setDepartureGateId(e.target.value)}>{gates.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select></label><label>Feltételek együtt<input type="number" min="0" value={departureHold} onChange={e=>setDepartureHold(e.target.value)}/><span>percig</span></label><div className="actionInfoBox danger"><strong>AND logika.</strong><span>A zárás csak akkor indul, ha a kiválasztott Wi-Fi eszköz offline ÉS a töltő kiválasztott állapota egyszerre teljesül.</span></div><label className="gateSafetyConfirmV22"><input type="checkbox" checked={gateSafetyConfirmed} onChange={e=>setGateSafetyConfirmed(e.target.checked)}/><span><b>Engedélyezem az automatikus fizikai kapumozgatást</b><small>A HomeHub ezt külön safety flaggel menti.</small></span></label></>}
    ...
    {template==="network_offline"&&<><label>Figyelt eszköz<select value={infraId} onChange={e=>setInfraId(e.target.value)}>{infra.map(n=><option key={n.id} value={n.id}>{n.name} · {n.ip||n.mac}</option>)}</select></label><label>Riasztás ennyi kiesés után<input type="number" min="1" value={offlineMinutes} onChange={e=>setOfflineMinutes(e.target.value)}/><span>perc</span></label></>}
    {template==="slow_link"&&<><label>Switch<select value={switchId} onChange={e=>setSwitchId(e.target.value)}>{managedSwitches.map(n=><option key={n.id} value={n.id}>{n.name}</option>)}</select></label><label>Port<input type="number" min="1" max="64" value={linkPort} onChange={e=>setLinkPort(e.target.value)}/></label><label>Küszöb<input type="number" min="10" step="10" value={linkThreshold} onChange={e=>setLinkThreshold(e.target.value)}/><span>Mbps alatt</span></label><label>Legalább<input type="number" min="0" value={linkHold} onChange={e=>setLinkHold(e.target.value)}/><span>percig</span></label>{managedSwitches.length===0&&<div className="actionInfoBox"><strong>Még nincs élő switch portadat.</strong><span>Add meg a TL-SG108E helyi admin jelszavát a WD Bridge network-secrets.json fájljában.</span></div>}</>}
  ...
  </div><div className="notificationComposerV22"><div className="notificationComposerHeadV22"><label className="accessSwitchV21"><input type="checkbox" checked={notifyEnabled} onChange={e=>setNotifyEnabled(e.target.checked)}/><span></span><b>{notifyEnabled?"Értesítés aktív":"Csak HomeHub log"}</b></label><select value={priority} onChange={e=>setPriority(e.target.value as NotificationPriority)}><option value="info">Info</option><option value="normal">Normál</option><option value="warning">Figyelmeztetés</option><option value="critical">Kritikus</option></select></div>{notifyEnabled&&<><strong>Címzettek</strong><div className="recipientGridV22">{people.map(p=><label key={p.id} className={recipientIds.includes(p.id)?"active":""}><input type="checkbox" checked={recipientIds.includes(p.id)} onChange={()=>toggleRecipient(p.id)}/><PersonAvatar person={p} size="small"/><span>{p.name}<small>{p.hasEmail||p.email?"email":"nincs email"} · {p.pushSubscriptionCount||0} push · {p.hasPhone||p.phone?"telefon":"nincs telefon"}</small></span></label>)}</div><strong>Csatornák</strong><div className="channelPickerV22">{(["push","email","sms"] as NotificationChannel[]).map(c=><label key={c} className={notifyChannels.includes(c)?"active":""}><input type="checkbox" checked={notifyChannels.includes(c)} onChange={()=>toggleChannel(c)}/><b>{c==="push"?"Push":c==="email"?"Email":"SMS"}</b></label>)}</div><label className="escalationToggleV22"><input type="checkbox" checked={escalate} onChange={e=>setEscalate(e.target.checked)}/><span></span><div><b>Eszkaláció, ha továbbra is fennáll</b><small>Például 10 perc után SMS is menjen.</small></div></label>{escalate&&<div className="escalationFieldsV22"><label>Ennyi idő után<input type="number" min="1" value={escalationMinutes} onChange={e=>setEscalationMinutes(e.target.value)}/><span>perc</span></label><div className="channelPickerV22">{(["push","email","sms"] as NotificationChannel[]).map(c=><label key={c} className={escalationChannels.includes(c)?"active":""}><input type="checkbox" checked={escalationChannels.includes(c)} onChange={()=>toggleChannel(c,true)}/><b>{c==="push"?"Push":c==="email"?"Email":"SMS"}</b></label>)}</div></div>}</>}</div><div className="actionComposerFooter"><button className="ghost" onClick={onClose}>Mégse</button><button className="primaryAction" disabled={busy||template==="vacuum_schedule"&&days.length===0} onClick={save}>{busy?"Mentés…":"Akció létrehozása"}</button></div></section></div>
}

function ActionsTab({state,smart,network,people,signals,vacuum,reload,flash}:{state:AutomationState;smart:SmartHome;network:NetworkStatus[];people:PersonProfile[];signals:ExternalSignal[];vacuum?:VacuumStatus;reload:()=>void;flash:(m:string)=>void}){
  const[composer,setComposer]=useState(false);const rules=state.rules||[],alerts=state.alerts||[];
  async function putRule(rule:AutomationRule,patch:Partial<AutomationRule>){const next={name:rule.name,enabled:rule.enabled,trigger:rule.trigger,actions:rule.actions,cooldownSeconds:rule.cooldownSeconds,notifyEmail:rule.notifyEmail!==false,notification:rule.notification,safety:rule.safety,...patch};try{await api(`/api/automations/${rule.id}`,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify(next)});reload()}catch(err){flash(`Akció hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}}
  ...
  async function toggleRule(rule:AutomationRule){if(!rule.enabled&&rule.safety?.allowGateAction&&!window.confirm(`${rule.name}\n\nEz a szabály fizikai kapumozgatást végezhet. Biztosan bekapcsolod?`))return;await putRule(rule,{enabled:!rule.enabled})}
  async function run(rule:AutomationRule){if(rule.safety?.allowGateAction&&!window.confirm(`${rule.name}\n\nA tesztfuttatás a feltételt megkerülve fizikailag mozgathatja a kaput. Biztosan futtatod?`))return;try{await api(`/api/automations/${rule.id}/run`,{method:"POST"});flash(`${rule.name}: tesztfuttatás elindítva`);setTimeout(reload,500)}catch(err){flash(`Teszt hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}}
  async function readAll(){await api("/api/alerts/read-all",{method:"POST"});reload()}
  return <div className="tabPanel actionsTab">{composer&&<AutomationComposer smart={smart} network={network} people={people} signals={signals} vacuum={vacuum} onClose={()=>setComposer(false)} onSaved={reload} flash={flash}/>}<section className="panel actionsHero"><div><span className="smartEyebrowV12">HOMEHUB AUTOMATION</span><h2>Akciók</h2><p>Feltétel, időzítés vagy hálózati esemény alapján automatikusan vezérelheted az otthonodat és riasztást kérhetsz.</p></div><button className="primaryAction" onClick={()=>setComposer(true)}>+ Új akció</button></section><section className="automationStats automationStatsV22"><div><span>Aktív</span><strong>{rules.filter(r=>r.enabled).length}</strong></div><div><span>Olvasatlan</span><strong>{state.unread||0}</strong></div><div><span>Push</span><strong>{state.notification?.pushConfigured?"Kész":"Nincs"}</strong><small>Web Push</small></div><div><span>Email</span><strong>{state.notification?.emailConfigured?"Kész":"Nincs"}</strong><small>SMTP</small></div><div><span>SMS</span><strong>{state.notification?.smsConfigured?"Kész":"Nincs"}</strong><small>Twilio</small></div></section>{signals.length>0&&<section className="panel signalStatusPanelV22"><div className="sectionHead"><div><h2>Külső jelek</h2><p>Geofence, BLE, ESP32 vagy más integráció legutóbbi jelei. Önmagukban nem indítanak semmit, csak az általad létrehozott automatizációk használják őket.</p></div><span className="sectionCounter">{signals.length}</span></div><div className="signalGridV22">{signals.slice(0,12).map(sig=><article key={sig.key}><div><span className={`signalKindV22 ${sig.category||"generic"}`}>{sig.category||"generic"}</span><strong>{sig.label||sig.key}</strong></div><b>{String(sig.value)}</b><small>{sig.key} · {new Date(sig.updatedAt).toLocaleString("hu-HU")}{sig.source?` · ${sig.source}`:""}</small></article>)}</div></section>}<section className="panel actionRulesPanel"><div className="sectionHead"><div><h2>Automatizálások</h2><p>A feltételes szabályok 10 másodperces motorral futnak; a hálózati állapot a WD Bridge frissítéseiből érkezik.</p></div></div>{rules.length===0?<div className="empty">Még nincs akció. A kész mintákból gyorsan létrehozhatod az elsőt.</div>:<div className="actionRuleList">{rules.map(rule=><article className={rule.enabled?"actionRule enabled":"actionRule"} key={rule.id}><div className="ruleStatus"><button className={rule.enabled?"ruleToggle on":"ruleToggle"} onClick={()=>toggleRule(rule)}><i></i></button></div><div className="ruleMain"><div className="ruleTitle"><strong>{rule.name}</strong><div className="ruleMetaV21 ruleNotifyMetaV22"><span className={`priorityBadgeV22 ${rule.notification?.priority||"normal"}`}>{rule.notification?.priority||"legacy"}</span><span>{rule.notification?.enabled?`${rule.notification.channels.map(c=>c==="push"?"Push":c==="email"?"Email":"SMS").join(" + ")} · ${rule.notification.recipientPersonIds.map(id=>people.find(p=>p.id===id)?.name||"?").join(", ")||"nincs címzett"}`:rule.notifyEmail!==false?"Legacy email":"Csak log"}</span>{rule.lastTriggeredAt&&<span>utoljára: {new Date(rule.lastTriggeredAt).toLocaleString("hu-HU")}</span>}</div></div><div className="ruleFlow"><span className="ruleWhen">HA</span><b>{triggerLabel(rule,smart,network,people,signals)}</b><span className="ruleThen">AKKOR</span><b>{rule.actions.map(a=>actionLabel(a,smart)).join(" + ")}</b></div></div><div className="ruleActions"><button onClick={()=>run(rule)}>Teszt</button><button className="deleteBtn" onClick={()=>remove(rule)}>Törlés</button></div></article>)}</div>}</section><section className="panel alertsPanel"><div className="sectionHead"><div><h2>Értesítések</h2><p>Az emailtől függetlenül minden riasztás megmarad a HomeHubban.</p></div>{state.unread>0&&<button className="ghost" onClick={readAll}>Mind olvasott</button>}</div>{alerts.length===0?<div className="empty">Még nincs riasztás.</div>:<div className="alertList">{alerts.slice(0,20).map(a=><article className={a.readAt?"alertItem":"alertItem unread"} key={a.id}><div className="alertIcon">{a.emailSent?"✉":"!"}</div><div><strong>{a.subject}</strong><p>{a.message}</p><small>{new Date(a.createdAt).toLocaleString("hu-HU")}{a.escalationLevel?` · eszkaláció ${a.escalationLevel}`:""} · {(a.deliveries||[]).length?(a.deliveries||[]).map(d=>`${d.personName||"fallback"} ${d.channel}: ${d.ok?"✓":d.skipped?"kihagyva":"hiba"}`).join(" · "):a.emailRequested?(a.emailSent?"email elküldve":`email: ${a.emailError||"nem sikerült"}`):"csak HomeHub"}</small></div></article>)}</div>}</section></div>
}


function Life360Settings({status,people,flash,reload}:{status?:Life360Status;people:PersonProfile[];flash:(m:string)=>void;reload:()=>void}){
  const [busy,setBusy]=useState(false);
  const st=status??{configured:false,online:false,experimental:true,lastUpdatedAt:null,members:[],mapping:{}};
  async function refresh(){setBusy(true);try{await api("/api/integrations/life360/refresh",{method:"POST"});reload();flash("Life360 frissítve.")}catch(e){flash(`Life360 hiba: ${e instanceof Error?e.message:"ismeretlen"}`)}finally{setBusy(false)}}
  async function map(memberId:string,personId:string){try{await api("/api/integrations/life360/mapping",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({memberId,personId})});reload()}catch(e){flash(`Mentési hiba: ${e instanceof Error?e.message:"ismeretlen"}`)}}
  return <section className="panel life360PanelV23"><div className="sectionHead"><div><span className="smartEyebrowV12">KÍSÉRLETI INTEGRÁCIÓ</span><h2>Life360</h2><p>A családi helyadatokat opcionális automatizációs jelekké alakítja. Semmilyen kapu- vagy eszközműveletet nem indít magától.</p></div><div className="life360ActionsV23"><span className={st.online?"statusBadge good":"statusBadge"}>{!st.configured?"Nincs konfigurálva":st.online?"Kapcsolódva":"Kapcsolati hiba"}</span><button className="ghost" disabled={!st.configured||busy} onClick={refresh}>{busy?"Frissítés…":"Frissítés"}</button></div></div>{st.error&&<div className="infoLine warning">{st.error}</div>}<div className="life360MetaV23"><span><small>Kör</small><b>{st.circleName||"—"}</b></span><span><small>Utolsó frissítés</small><b>{st.lastUpdatedAt?new Date(st.lastUpdatedAt).toLocaleString("hu-HU"):"—"}</b></span><span><small>Tagok</small><b>{st.members.length}</b></span></div>{st.members.length>0&&<div className="life360MembersV23">{st.members.map(m=>{const name=[m.firstName,m.lastName].filter(Boolean).join(" ")||m.id;return <article key={m.id}><div><strong>{name}</strong><small>{m.location?.name||m.loginEmail||m.id}</small></div><select value={st.mapping?.[m.id]||""} onChange={e=>map(m.id,e.target.value)}><option value="">Automatikus / nincs hozzárendelve</option>{people.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select><div className="life360MemberStatsV23"><span>Akku <b>{m.location?.battery??"—"}%</b></span><span>Sebesség <b>{m.location?.speed??"—"}</b></span></div></article>})}</div>}<p className="personNoteV19">A Life360 API nem hivatalos és változhat. Hozzáférési adatokat csak Render Environment változóban tárolunk; a Home Hub felület nem jeleníti meg őket.</p></section>
}
...
function AIAssistantTab({ai,settings,smart,network,reload,flash,updateSettings}:{ai:AIState;settings:State["settings"];smart:SmartHome;network:NetworkStatus[];reload:()=>void;flash:(m:string)=>void;updateSettings:(p:Partial<State["settings"]>)=>Promise<void>}){
  const[messages,setMessages]=useState<AIChatMessage[]>([{role:"assistant",text:"Kérdezz rá az otthon aktuális vagy korábbi állapotára: ki van itthon, melyik gép volt online éjjel, mikor nyílt a kapu; kérhetsz automatizálási tervet vagy jóváhagyható gyors parancsot is."}]);
...
function DeviceIdentityEditor({device,onClose,onSaved,flash}:{device:NetworkStatus;onClose:()=>void;onSaved:()=>void;flash:(m:string)=>void}){
  const[name,setName]=useState(device.identity?.source==="manual"?device.name:(device.identity?.label||device.name||""));
  const[kind,setKind]=useState(device.identity?.deviceType||"");
  const[owner,setOwner]=useState(device.identity?.owner||"");
  const[note,setNote]=useState(device.identity?.source==="manual"?(device.identity?.note||""):"");
  const[busy,setBusy]=useState(false);
  useEffect(()=>{setName(device.identity?.source==="manual"?device.name:(device.identity?.label||device.name||""));setKind(device.identity?.deviceType||"");setOwner(device.identity?.owner||"");setNote(device.identity?.source==="manual"?(device.identity?.note||""):"")},[device.id,device.mac]);
  async function save(){if(!device.mac||!name.trim())return;setBusy(true);try{await api(`/api/network/identity/${encodeURIComponent(device.mac)}`,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({name:name.trim(),kind:kind.trim(),owner:owner.trim(),note:note.trim()||"HomeHub felületen megerősítve."})});flash("Eszközazonosítás mentve");onSaved();onClose()}catch(err){flash(`Azonosítás hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}finally{setBusy(false)}}
  return <section className="panel identityEditorV211"><div className="sectionHead"><div><span className="smartEyebrowV12">DEVICE IDENTITY</span><h2>Eszköz szerkesztése</h2><p>Az azonosítás nem felugró ablakban történik. A mentett név MAC-alapon követi az eszközt IP-változás után is.</p></div><button className="ghost" onClick={onClose}>Bezárás</button></div><div className="identitySummaryV211"><span><small>Aktuális IP</small><b>{device.ip||"—"}</b></span><span><small>MAC</small><b>{device.mac||"—"}</b></span><span><small>Automatikus név</small><b>{device.rawName||device.name}</b></span><span><small>Forrás</small><b>{device.identity?.source||"felderítés"}</b></span></div><div className="identityFieldsV211"><label>Eszköz neve<input value={name} onChange={e=>setName(e.target.value)} placeholder="pl. Lenti nappali Telekom TV beltéri"/></label><label>Eszköztípus<input value={kind} onChange={e=>setKind(e.target.value)} placeholder="telefon, TV beltéri, okoskonnektor…"/></label><label>Tulajdonos<input value={owner} onChange={e=>setOwner(e.target.value)} placeholder="Opcionális"/></label><label className="identityNoteV211">Megjegyzés<input value={note} onChange={e=>setNote(e.target.value)} placeholder="Opcionális megjegyzés"/></label></div><div className="identityActionsV211"><button className="ghost" onClick={onClose}>Mégse</button><button className="primaryAction" disabled={busy||!name.trim()} onClick={save}>{busy?"Mentés…":"Azonosítás mentése"}</button></div></section>
}

function PersonAvatar({person,size="normal"}:{person:PersonProfile;size?:"small"|"normal"|"large"}){
  const initials=person.name.split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]?.toUpperCase()).join("")||"?";
  return <div className={`personAvatarV19 ${size}`}>{person.hasAvatar?<img src={`/api/people/${encodeURIComponent(person.id)}/avatar?v=${encodeURIComponent(person.updatedAt)}`} alt=""/>:<span>{initials}</span>}</div>
}

function PresenceStrip({people,presence,onOpen}:{people:PersonProfile[];presence:PresenceStatus[];onOpen:()=>void}){
  if(!people.length)return <section className="panel presenceEmptyV19"><div><span className="smartEyebrowV12">PEOPLE & PRESENCE</span><h2>Ki van itthon?</h2><p>Vegyél fel családtagokat, és rendeld hozzájuk a telefonokat vagy más hálózati eszközöket.</p></div><button className="primaryAction" onClick={onOpen}>Emberek felvétele</button></section>;
  const byId=new Map(presence.map(x=>[x.personId,x]));
  return <section className="panel presenceStripV19"><div className="presenceStripHeadV19"><div><span className="smartEyebrowV12">PEOPLE & PRESENCE</span><h2>Ki van itthon?</h2></div><button className="ghost" onClick={onOpen}>Kezelés</button></div><div className="presencePeopleV19">{people.map(p=>{const ps=byId.get(p.id),home=ps?.status==="home",away=ps?.status==="away";return <button key={p.id} className={`presencePersonV19 ${home?"home":away?"away":"uncertain"}`} onClick={onOpen}><PersonAvatar person={p}/><div><strong>{p.nickname||p.name}</strong><span>{home?"Itthon":away?"Nincs itthon":"Bizonytalan"}</span><small>{ps?.source||ps?.note||"Nincs jelenléti adat"}</small></div><i>{home?"✓":away?"×":"?"}</i></button>})}</div></section>
}

function PeopleTab({people,presence,network,reload,flash,currentUser}:{people:PersonProfile[];presence:PresenceStatus[];network:NetworkStatus[];reload:()=>void;flash:(m:string)=>void;currentUser:AuthUser}){
  const[editing,setEditing]=useState<PersonProfile|null>(null),[creating,setCreating]=useState(false),[name,setName]=useState(""),[nickname,setNickname]=useState(""),[role,setRole]=useState("Családtag"),[email,setEmail]=useState(""),[phone,setPhone]=useState(""),[pushEnabled,setPushEnabled]=useState(true),[emailEnabled,setEmailEnabled]=useState(true),[smsEnabled,setSmsEnabled]=useState(false),[links,setLinks]=useState<Record<string,PersonDeviceRole|"">>({}),[busy,setBusy]=useState(false),[accessBusy,setAccessBusy]=useState(false);
  const[accessEnabled,setAccessEnabled]=useState(false),[loginName,setLoginName]=useState(""),[accessPassword,setAccessPassword]=useState(""),[permissions,setPermissions]=useState<MenuPermission[]>(["overview"]);
  const byPresence=new Map(presence.map(x=>[x.personId,x]));
  const assignable=network.filter(n=>["computer","discovered"].includes(n.kind)).sort((a,b)=>a.name.localeCompare(b.name,"hu"));
  const permissionDefs=tabDefs.map(t=>({id:t.id as MenuPermission,label:t.label}));
  function reset(){setEditing(null);setCreating(false);setName("");setNickname("");setRole("Családtag");setEmail("");setPhone("");setPushEnabled(true);setEmailEnabled(true);setSmsEnabled(false);setLinks({});setAccessEnabled(false);setLoginName("");setAccessPassword("");setPermissions(["overview"])}
  function startNew(){reset();setCreating(true)}
  function applyPerson(p:PersonProfile){setEditing(p);setCreating(true);setName(p.name);setNickname(p.nickname||"");setRole(p.role||"Családtag");setEmail(p.email||"");setPhone(p.phone||"");setPushEnabled(p.notificationPrefs?.pushEnabled!==false);setEmailEnabled(p.notificationPrefs?.emailEnabled!==false);setSmsEnabled(Boolean(p.notificationPrefs?.smsEnabled));setLinks(Object.fromEntries(p.devices.map(d=>[d.networkId,d.role])));setAccessEnabled(Boolean(p.auth?.enabled));setLoginName(p.auth?.loginName||"");setAccessPassword("");setPermissions(p.auth?.permissions?.length?p.auth.permissions:["overview"])}
  function startEdit(p:PersonProfile){applyPerson(p)}
  function togglePermission(id:MenuPermission){setPermissions(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id])}
  function profileBody(){const devices=Object.entries(links).filter(([,r])=>Boolean(r)).map(([networkId,r])=>({networkId,role:r as PersonDeviceRole,label:network.find(n=>n.id===networkId)?.name||""}));return {name:name.trim(),nickname:nickname.trim(),role:role.trim(),email:email.trim(),phone:phone.trim(),notificationPrefs:{pushEnabled,emailEnabled,smsEnabled},devices}}
  async function saveProfile(){if(!name.trim())return;setBusy(true);try{const saved=await api(editing?`/api/people/${editing.id}`:"/api/people",{method:editing?"PUT":"POST",headers:{"content-type":"application/json"},body:JSON.stringify(profileBody())}) as PersonProfile;flash(editing?"Profil frissítve":"Személy felvéve. Most beállítható a hozzáférése is.");applyPerson(saved);reload()}catch(err){flash(`Profil hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}finally{setBusy(false)}}
  async function saveAccess(){if(!editing){flash("Előbb mentsd a személy profilját.");return}const effectiveLogin=loginName.trim()||(accessEnabled?(nickname.trim()||name.trim()).toLocaleLowerCase("hu-HU").replace(/\s+/g,"."):"");if(accessEnabled&&effectiveLogin.length<2){flash("Adj meg felhasználónevet.");return}if(accessEnabled&&!editing.auth?.hasPassword&&accessPassword.length<8){flash("Az első hozzáféréshez legalább 8 karakteres jelszó szükséges.");return}if(accessPassword&&accessPassword.length<8){flash("A jelszó legalább 8 karakter legyen.");return}setAccessBusy(true);try{const updated=await api(`/api/people/${editing.id}/access`,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({enabled:accessEnabled,loginName:effectiveLogin,password:accessPassword,permissions:permissions.length?permissions:["overview"]})}) as PersonProfile;flash("Webes hozzáférés mentve");applyPerson(updated);reload()}catch(err){flash(`Hozzáférés hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}finally{setAccessBusy(false)}}
  async function remove(p:PersonProfile){if(!window.confirm(`Törlöd ${p.name} profilját?`))return;await api(`/api/people/${p.id}`,{method:"DELETE"});reload()}
  async function avatar(p:PersonProfile,file?:File){if(!file)return;try{let payload:Blob=file;if(file.size>600*1024){const bitmap=await createImageBitmap(file);const max=320,scale=Math.min(1,max/Math.max(bitmap.width,bitmap.height));const canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));const ctx=canvas.getContext("2d");if(!ctx)throw new Error("image_canvas_unavailable");ctx.drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close();payload=await new Promise<Blob>((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error("image_encode_failed")),"image/jpeg",.82))}if(payload.size>700*1024)throw new Error("avatar_too_large_after_resize");const fd=new FormData();fd.append("avatar",payload,"avatar.jpg");await api(`/api/people/${p.id}/avatar`,{method:"POST",body:fd});flash("Profilkép frissítve");reload()}catch(err){flash(`Képfeltöltés hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}}
  return <div className="tabPanel peopleTabV19"><section className="panel peopleHeroV19"><div><span className="smartEyebrowV12">HOUSEHOLD</span><h2>Emberek és hozzáférések</h2><p>Jelenlét, személyes eszközök és külön HomeHub belépés egy helyen. A profil, a hozzáférés és az eszköz-hozzárendelés ugyanazon az oldalon szerkeszthető.</p></div>{currentUser.isAdmin&&<button className="primaryAction" onClick={startNew}>+ Személy felvétele</button>}</section>
  <section className="peopleGridV19">{people.map(p=>{const ps=byPresence.get(p.id);return <article className="panel personCardV19" key={p.id}><div className="personHeadV19"><PersonAvatar person={p} size="large"/><div><strong>{p.name}</strong><span>{p.role||"Családtag"}</span><b className={`presenceBadgeV19 ${ps?.status||"uncertain"}`}>{ps?.status==="home"?"Itthon":ps?.status==="away"?"Nincs itthon":"Bizonytalan"} · {ps?.confidence??0}%</b></div></div><div className="personMetaV19"><span><small>Webes hozzáférés</small><b>{p.auth?.enabled?`Aktív · ${p.auth.loginName}`:"Nincs bekapcsolva"}</b></span><span><small>Értesítés</small><b>{[p.notificationPrefs?.pushEnabled!==false&&"Push",p.notificationPrefs?.emailEnabled!==false&&(p.hasEmail||p.email)&&"Email",p.notificationPrefs?.smsEnabled&&(p.hasPhone||p.phone)&&"SMS"].filter(Boolean).join(" · ")||"Nincs"}</b></span><span><small>Push eszköz</small><b>{p.pushSubscriptionCount||0}</b></span></div><div className="personDevicesV19">{p.devices.length?p.devices.map(d=>{const n=network.find(x=>x.id===d.networkId);return <span key={d.networkId}><i className={n?.online?"online":""}></i>{d.label||n?.name||d.networkId}<small>{d.role==="primary"?"elsődleges":d.role==="secondary"?"másodlagos":"stacionárius"}</small></span>}):<em>Nincs eszköz hozzárendelve.</em>}</div>{currentUser.isAdmin&&<div className="personActionsV19"><label className="ghost fileGhostV19">Kép feltöltése<input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>avatar(p,e.target.files?.[0])}/></label><button className="ghost" onClick={()=>startEdit(p)}>Szerkesztés</button><button className="deleteBtn" onClick={()=>remove(p)}>Törlés</button></div>}{ps?.note&&<p className="personNoteV19">{ps.note}</p>}</article>})}{!people.length&&<section className="panel empty">Még nincs személy felvéve.</section>}</section>
  {creating&&<section className="panel personEditorV19"><div className="sectionHead"><div><h2>{editing?`${editing.name} szerkesztése`:"Új személy"}</h2><p>A telefon legyen az elsődleges jelenléti eszköz. Minden beállítás ezen az oldalon marad, nincs felugró szerkesztőablak.</p></div><button className="ghost" onClick={reset}>Bezárás</button></div><div className="personFormV19"><label>Név<input value={name} onChange={e=>setName(e.target.value)} placeholder="Név"/></label><label>Becenév<input value={nickname} onChange={e=>setNickname(e.target.value)} placeholder="Opcionális"/></label><label>Szerep<input value={role} onChange={e=>setRole(e.target.value)} placeholder="Családtag"/></label></div><div className="contactEditorV22"><div><span className="smartEyebrowV12">ÉRTESÍTÉSEK</span><h3>Kapcsolattartás</h3><p>Az automatizálások címzettként a személyt használják, nem egy globális email címet.</p></div><div className="contactFieldsV22"><label>Email<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="nev@example.com"/></label><label>Telefonszám<input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="+36 30 123 4567"/></label></div><div className="channelPrefsV22"><label className={pushEnabled?"active":""}><input type="checkbox" checked={pushEnabled} onChange={e=>setPushEnabled(e.target.checked)}/><b>Push</b><small>A saját telefonján külön engedélyezendő</small></label><label className={emailEnabled?"active":""}><input type="checkbox" checked={emailEnabled} onChange={e=>setEmailEnabled(e.target.checked)}/><b>Email</b><small>A fenti email címre</small></label><label className={smsEnabled?"active":""}><input type="checkbox" checked={smsEnabled} onChange={e=>setSmsEnabled(e.target.checked)}/><b>SMS</b><small>Csak Twilio konfiguráció esetén</small></label></div></div><div className="profileSaveRowV211"><span>{editing?"A profiladatok és eszköz-hozzárendelések külön menthetők a belépési jogosultságtól.":"Előbb mentsd a profilt, utána állítható be a személyes belépés."}</span><button className="primaryAction" disabled={busy||!name.trim()} onClick={saveProfile}>{busy?"Mentés…":editing?"Profil mentése":"Profil létrehozása"}</button></div>
    {currentUser.isAdmin&&<div className="accessEditorV21"><div className="accessEditorHeadV21"><div><span className="smartEyebrowV12">WEB ACCESS</span><h3>Személyes belépés</h3><p>Külön felhasználónév, jelszó és menüjogosultságok. A hozzáférés saját Mentés gombot kapott.</p></div><label className="accessSwitchV21"><input type="checkbox" checked={accessEnabled} onChange={e=>setAccessEnabled(e.target.checked)}/><span></span><b>{accessEnabled?"Aktív":"Kikapcsolva"}</b></label></div><div className="accessFieldsV21"><label>Felhasználónév<input value={loginName} onChange={e=>setLoginName(e.target.value)} placeholder={nickname||name||"pl. dorka"}/></label><label>{editing?.auth?.hasPassword?"Új jelszó (ha változik)":"Jelszó"}<input type="password" value={accessPassword} onChange={e=>setAccessPassword(e.target.value)} placeholder={editing?.auth?.hasPassword?"Hagyd üresen a megtartáshoz":"Legalább 8 karakter"}/></label></div><div className="permissionHeadV21"><strong>Látható menüpontok</strong><div><button type="button" className="ghost" onClick={()=>setPermissions(permissionDefs.map(x=>x.id))}>Mind</button><button type="button" className="ghost" onClick={()=>setPermissions(["overview","people","timeline","smart","actions"])}>Alap</button></div></div><div className="permissionGridV21">{permissionDefs.map(x=><label key={x.id} className={permissions.includes(x.id)?"active":""}><input type="checkbox" checked={permissions.includes(x.id)} onChange={()=>togglePermission(x.id)}/><span>{x.label}</span></label>)}</div><div className="accessSaveRowV211"><small>{!editing?"A hozzáférés mentéséhez előbb hozd létre a profilt.":accessEnabled&&!editing.auth?.hasPassword?"Első bekapcsoláskor legalább 8 karakteres jelszó kell.":"A jelszó üresen hagyva változatlan marad."}</small><button className="primaryAction" disabled={accessBusy||!editing} onClick={saveAccess}>{accessBusy?"Mentés…":"Hozzáférés mentése"}</button></div></div>}
    <h3>Eszközök hozzárendelése</h3><div className="deviceAssignV19">{assignable.map(n=><article key={n.id}><div><span className={`deviceDot ${n.online?"on":""}`}></span><strong>{n.name}</strong><small>{n.ip||n.mac||"Nincs IP"}</small></div><select value={links[n.id]||""} onChange={e=>setLinks(v=>({...v,[n.id]:e.target.value as PersonDeviceRole|""}))}><option value="">Nincs hozzárendelve</option><option value="primary">Elsődleges jelenlét</option><option value="secondary">Másodlagos személyes</option><option value="stationary">Otthon maradhat (PC/laptop)</option></select></article>)}</div>{assignable.length===0&&<div className="infoLine">Nincs hozzárendelhető kliens a Bridge pillanatképében.</div>}<div className="personEditorActionsV19"><button className="ghost" onClick={reset}>Bezárás</button>{editing&&<button className="primaryAction" disabled={busy} onClick={saveProfile}>{busy?"Mentés…":"Eszközök és profil mentése"}</button>}</div></section>}
  </div>
}

function TimelineTab({events}:{events:HistoryEvent[]}){
  const[filter,setFilter]=useState<HistoryEvent["category"]|"all">("all");
  const visible=events.filter(e=>filter==="all"||e.category===filter);
  const icon=(c:string)=>c==="presence"?"●":c==="security"?"⌂":c==="network"?"⌁":c==="energy"?"⚡":c==="automation"?"▶":c==="smart"?"◇":"•";
  return <div className="tabPanel"><section className="panel timelineHeroV19"><div><span className="smartEyebrowV12">HOME TIMELINE</span><h2>Otthon idővonala</h2><p>Hálózati, jelenléti és kapuesemények. A részletes előzmény 90 napig marad meg, a WD állapotmentésében is tükrözve.</p></div><div className="timelineFiltersV19">{(["all","presence","security","network","smart","energy","automation","system"] as const).map(x=><button key={x} className={filter===x?"active":""} onClick={()=>setFilter(x)}>{x==="all"?"Minden":x==="presence"?"Jelenlét":x==="security"?"Biztonság":x==="network"?"Hálózat":x==="smart"?"Smart":x==="energy"?"Energia":x==="automation"?"Akciók":"Rendszer"}</button>)}</div></section><section className="panel timelinePanelV19">{visible.length?<div className="homeTimelineV19">{visible.map(e=><article key={e.id}><div className={`timelineIconV19 ${e.category}`}>{icon(e.category)}</div><div><strong>{e.message}</strong><span>{e.entityName||e.type}</span></div><time>{new Date(e.createdAt).toLocaleString("hu-HU")}</time></article>)}</div>:<div className="empty">Ehhez a szűrőhöz még nincs esemény.</div>}</section></div>
}

function CredentialsTab({vault}:{vault?:VaultStatus}){
  if(!vault)return <div className="tabPanel"><section className="panel"><div className="empty">A Bridge még nem küldött Credentials Vault állapotot.</div></section></div>;
  const saved=vault.entries.filter(e=>e.saved&&e.hasPassword).length;
  return <div className="tabPanel credentialsV18">
    <section className="panel credentialsHeroV18"><div className="sectionHead"><div><span className="smartEyebrowV12">LOCAL CREDENTIALS VAULT</span><h2>Hozzáférések</h2><p>Routerek, switch, erősítők és NAS-ok admin elérései egy helyen. A Home Hub felhős része csak azt látja, hogy van-e mentett jelszó; a jelszó maga kizárólag a WD My Cloud titkosított trezorában marad.</p></div><div className="credentialSummaryV18"><strong>{saved}/{vault.entries.length}</strong><span>jelszó mentve</span><b>{vault.pinConfigured?"PIN aktív":"PIN szükséges"}</b></div></div>
      <div className="vaultActionsV18">{vault.localUrl&&<a className="actionLink vaultPrimaryV18" href={vault.localUrl} target="_blank" rel="noreferrer">Helyi trezor megnyitása</a>}<span className={vault.initialized?"statusBadge good":"statusBadge"}>{vault.initialized?"Titkosított trezor aktív":"Trezor hiba"}</span></div>
      {!vault.pinConfigured&&<div className="infoLine">Első használatkor a helyi trezor oldalon állíts be legalább 6 karakteres PIN-t. A régi TL-SG108E network-secrets.json tartalma sikeres migráció után automatikusan átkerül a titkosított trezorba.</div>}
    </section>
    <section className="panel"><div className="sectionHead"><div><h2>Admin hozzáférések</h2><p>Az admin cím és a felhasználónév megjelenhet itt, a jelszó soha. A jelszó megtekintése és módosítása a helyi WD trezorban történik.</p></div></div><div className="credentialGridV18">{vault.entries.map(e=><article className="credentialCardV18" key={e.id}><div className="deviceHead"><div><strong>{e.label}</strong></div><span className={e.saved&&e.hasPassword?"deviceKind credentialOkV18":"deviceKind"}>{e.saved&&e.hasPassword?"Mentve":"Nincs jelszó"}</span></div><small>{e.kind||"Eszköz"}{e.ip?` · ${e.ip}`:""}</small><div className="credentialFieldsV18"><span><small>Felhasználó</small><b>{e.username||"—"}</b></span><span><small>Jelszó</small><b>{e.hasPassword?"••••••••":"—"}</b></span></div><div className="credentialActionsV18">{e.adminUrl&&<a href={e.adminUrl} target="_blank" rel="noreferrer">Admin megnyitása</a>}{vault.localUrl&&<a href={vault.localUrl} target="_blank" rel="noreferrer">Trezor</a>}</div></article>)}</div></section>
    <section className="panel"><div className="infoLine">Biztonsági modell: AES-GCM titkosított credentials.vault + külön root-only vault.key, 600-as jogosultságok, helyi PIN-es reveal munkamenet. Render/OpenAI csak metaadatot kap. Teljes WD root kompromittálás ellen ez nem különálló hardveres trezor.</div></section>
  </div>
}


function base64UrlToUint8Array(value:string){const padding="=".repeat((4-value.length%4)%4),base64=(value+padding).replace(/-/g,"+").replace(/_/g,"/");const raw=atob(base64);return Uint8Array.from([...raw].map(c=>c.charCodeAt(0)))}
function PushNotificationSettings({currentUser,status,flash,reload}:{currentUser:AuthUser;status:AutomationState["notification"];flash:(m:string)=>void;reload:()=>void}){
  const[subscribed,setSubscribed]=useState(false),[busy,setBusy]=useState(false),[permission,setPermission]=useState<NotificationPermission>(typeof Notification!=="undefined"?Notification.permission:"default");
  useEffect(()=>{if(!("serviceWorker" in navigator)||!("PushManager" in window))return;navigator.serviceWorker.ready.then(r=>r.pushManager.getSubscription()).then(s=>setSubscribed(Boolean(s))).catch(()=>{})},[]);
  async function enable(){if(!currentUser.personId)return flash("Push csak személyhez kötött belépéssel kapcsolható be.");if(!("serviceWorker" in navigator)||!("PushManager" in window)||typeof Notification==="undefined")return flash("Ez a böngésző nem támogatja a Web Push értesítéseket.");setBusy(true);try{const perm=await Notification.requestPermission();setPermission(perm);if(perm!=="granted")throw new Error("push_permission_denied");const key=await api("/api/notifications/push/public-key") as {publicKey:string};const reg=await navigator.serviceWorker.ready;let sub=await reg.pushManager.getSubscription();if(!sub)sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:base64UrlToUint8Array(key.publicKey)});const json=sub.toJSON();await api("/api/notifications/push/subscribe",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({endpoint:sub.endpoint,expirationTime:sub.expirationTime,keys:json.keys})});setSubscribed(true);flash("Push értesítés bekapcsolva ezen az eszközön.");reload()}catch(err){flash(`Push hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}finally{setBusy(false)}}
  async function disable(){setBusy(true);try{const reg=await navigator.serviceWorker.ready,sub=await reg.pushManager.getSubscription();if(sub){await api("/api/notifications/push/subscribe",{method:"DELETE",headers:{"content-type":"application/json"},body:JSON.stringify({endpoint:sub.endpoint})});await sub.unsubscribe()}setSubscribed(false);flash("Push értesítés kikapcsolva ezen az eszközön.");reload()}catch(err){flash(`Push hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}finally{setBusy(false)}}
  return <section className="panel pushSettingsV22"><div><span className="smartEyebrowV12">PUSH ÉRTESÍTÉS</span><h2>Ez az eszköz</h2><p>A push előfizetés mindig ahhoz a személyhez kötődik, aki ezen az eszközön be van jelentkezve.</p></div><div className="pushStatusV22"><span className={status?.pushConfigured?"statusBadge good":"statusBadge"}>{status?.pushConfigured?"Szerver kész":"VAPID nincs beállítva"}</span><span className={subscribed?"statusBadge good":"statusBadge"}>{subscribed?"Ez az eszköz feliratkozott":permission==="denied"?"Böngészőben tiltva":"Nincs feliratkozva"}</span></div>{currentUser.personId?<div className="pushActionsV22">{subscribed?<button className="ghost" disabled={busy} onClick={disable}>Push kikapcsolása</button>:<button className="primaryAction" disabled={busy||!status?.pushConfigured} onClick={enable}>{busy?"Beállítás…":"Push bekapcsolása"}</button>}</div>:<div className="infoLine">Az admin technikai fiókhoz nincs személy rendelve. Pushhoz jelentkezz be a saját, például Béla felhasználóddal.</div>}</section>
}

function App(){
  const[auth,setAuth]=useState<"checking"|"yes"|"no">("checking"),[currentUser,setCurrentUser]=useState<AuthUser|null>(null),[state,setState]=useState<State|null>(null),[mediaLibrary,setMediaLibrary]=useState<MediaSnapshot|undefined>(),[magnet,setMagnet]=useState(""),[busy,setBusy]=useState(false),[notice,setNotice]=useState(""),[deleteTarget,setDeleteTarget]=useState<Torrent|null>(null),[detailTarget,setDetailTarget]=useState<TuyaDevice|null>(null),[vacuumDetail,setVacuumDetail]=useState(false),[identityEditing,setIdentityEditing]=useState<NetworkStatus|null>(null),[tab,setTab]=useState<Tab>(initialTab),[smartFilter,setSmartFilter]=useState<SmartFilter>("all"),[smartQuery,setSmartQuery]=useState(""),[smartOnlineOnly,setSmartOnlineOnly]=useState(false);
  async function checkAuth(){const r=await fetch("/api/auth/status");const j=await r.json();setCurrentUser(j.user||null);setAuth(j.authenticated?"yes":"no")}
  ...
  useEffect(()=>{const savedTheme=localStorage.getItem("homehub-theme");if(savedTheme==="light"||savedTheme==="dark")document.documentElement.dataset.theme=savedTheme;checkAuth();if("serviceWorker"in navigator)navigator.serviceWorker.register("/sw.js",{updateViaCache:"none"}).then(r=>r.update()).catch(()=>{})},[]);
  ...
  useEffect(()=>{if(auth!=="yes"||!currentUser)return;if(!currentUser.isAdmin&&!currentUser.permissions.includes(tab as MenuPermission)){const first=(currentUser.permissions[0]||"overview") as Tab;setTab(first);window.history.replaceState(null,"",`#${first}`)}},[auth,currentUser,tab]);
  const torrents=state?.snapshot?.kd20.torrents||[],printer=state?.snapshot?.printer,network=state?.snapshot?.network||[],vacuum=state?.snapshot?.vacuum,mediaSummary=state?.snapshot?.media,media=mediaLibrary??mediaSummary,vault=state?.snapshot?.vault,people=state?.people||[],presence=state?.presence||[],timeline=state?.timeline||[],signals=state?.signals||[],smart:SmartHome=state?.smartHome??{configured:false,online:false,lastUpdatedAt:null,devices:[],scenes:[]},automation:AutomationState=state?.automation??{rules:[],alerts:[],unread:0,email:{configured:false,recipients:0},notification:{emailConfigured:false,fallbackEmailRecipients:0,pushConfigured:false,smsConfigured:false}},ai:AIState=state?.ai??{configured:false,model:"gpt-5",mode:state?.settings?.aiMode||"suggest",policy:"confirm-before-execute"};
  ...
  const mobileTabs=mobilePriority.map(id=>allowedTabs.find(t=>t.id===id)).filter(Boolean) as typeof tabDefs;
  function chooseTab(next:Tab){if(!currentUser)return;if(!currentUser.isAdmin&&!currentUser.permissions.includes(next as MenuPermission))return;setTab(next);window.history.replaceState(null,"",`#${next}`);window.scrollTo({top:0,behavior:"smooth"})}
  ...
  function identifyNetwork(n:NetworkStatus){if(!n.mac)return;setIdentityEditing(n);window.setTimeout(()=>document.querySelector(".identityEditorV211")?.scrollIntoView({behavior:"smooth",block:"start"}),30)}
  ...
  async function logout(){await fetch("/api/auth/logout",{method:"POST"});setState(null);setMediaLibrary(undefined);setCurrentUser(null);setAuth("no")}
  if(auth==="checking")return<main className="splash"><div className="brandMark">H</div><p>HomeHub betöltése…</p></main>;
  if(auth==="no")return<Login onDone={(user)=>{setCurrentUser(user);setAuth("yes")}}/>;
  if(!currentUser)return<main className="splash"><div className="brandMark">H</div><p>Jogosultságok betöltése…</p></main>;
  ...
  return <div className="hhApp">{notice&&<div className="toast">{notice}</div>}{deleteTarget&&<DeleteDialog torrent={deleteTarget} onClose={()=>setDeleteTarget(null)} onDelete={removeTorrent}/>} {resolvedDetail&&<DeviceDetailDialog device={resolvedDetail} onClose={()=>setDetailTarget(null)} onCommand={smartCommand}/>} {vacuumDetail&&vacuum&&<VacuumDetailDialog vacuum={vacuum} onClose={()=>setVacuumDetail(false)} onAction={vacuumAction}/>}
    <aside className="hhSidebar">
      <button className="hhBrand" onClick={()=>chooseTab("overview")}><span className="hhBrandMark">⌂</span><span>HomeHub</span></button>
      <nav className="hhNav hhDesktopNav" aria-label="HomeHub funkciók">{allowedTabs.map(t=><button key={t.id} className={tab===t.id?"active":""} onClick={()=>chooseTab(t.id)}><span className="hhNavIcon">{t.id==="overview"?"⌂":t.id==="people"?"◉":t.id==="timeline"?"◷":t.id==="downloads"?"↓":t.id==="media"?"▶":t.id==="smart"?"◇":t.id==="actions"?"⚡":t.id==="ai"?"✦":t.id==="network"?"⌁":t.id==="credentials"?"🔑":t.id==="printer"?"▣":"⚙"}</span><span>{t.label}</span>{t.id==="actions"&&automation.unread>0&&<i className="navAlertBadge">{automation.unread}</i>}</button>)}</nav><nav className="hhMobileNav" aria-label="Gyors navigáció">{mobileTabs.map(t=><button key={t.id} className={tab===t.id?"active":""} onClick={()=>chooseTab(t.id)}><span>{t.id==="overview"?"⌂":t.id==="people"?"◉":t.id==="smart"?"◇":t.id==="actions"?"⚡":"⌁"}</span><small>{t.short}</small>{t.id==="actions"&&automation.unread>0&&<i className="navAlertBadge">{automation.unread}</i>}</button>)}</nav>
      <div className="hhSidebarBottom"><div className="hhUserPill"><span>{currentUser.name.slice(0,1).toUpperCase()}</span><div><b>{currentUser.name}</b><small>{currentUser.isAdmin?"Adminisztrátor":`${currentUser.permissions.length} menüpont`}</small></div></div><div className="hhSystemPill"><span className={state?.bridgeOnline?"dot on":"dot"}></span><div><b>{state?.bridgeOnline?"Otthon online":"Bridge offline"}</b><small>{state?.bridgeOnline?"Minden rendszer elérhető":bridgeAge(state?.bridgeLastSeenAt)}</small></div></div><button className="hhLogout" onClick={logout}>Kilépés</button></div>
    </aside>
    <main className="hhMain">
      <header className="hhTopbar"><div><span className="hhEyebrow">HOME HUB · CORE</span><h1>{tabDefs.find(t=>t.id===tab)?.label||"HomeHub"}</h1></div><div className="hhTopActions"><select className="hhMobileTabSelect" value={tab} onChange={e=>chooseTab(e.target.value as Tab)}>{allowedTabs.map(t=><option key={t.id} value={t.id}>{t.label}</option>)}</select><span className="hhTopStatus"><i className={state?.bridgeOnline?"dot on":"dot"}></i>{state?.bridgeOnline?"Bridge online":"Bridge offline"}</span><button className="hhTheme" aria-label="Megjelenés" onClick={()=>{const next=document.documentElement.dataset.theme==="light"?"dark":"light";document.documentElement.dataset.theme=next;localStorage.setItem("homehub-theme",next)}}>☼</button></div></header>
      {state?.persistence?.bootstrapPending&&<div className="stateSyncBannerV23"><b>Állapot visszaállítása folyamatban</b><span>A WD Bridge állapotának betöltéséig a szerkesztés átmenetileg le van tiltva.</span></div>}

    {tab==="overview"&&<div className="tabPanel hhOverview">
      <section className="hhHomeHero">
        <div className="hhHeroImage"></div><div className="hhHeroWash"></div>
        <div className="hhHeroInner">
          <div className="hhHeroTop"><div><span className="hhHeroKicker">OTTHON</span><h2>Jó estét!</h2><p>{network.filter(n=>n.online).length} hálózati eszköz online · {onlineSmart}/{smartDeviceTotal} okoseszköz elérhető</p></div><div className="hhHeroChips"><span>WD {state?.snapshot?.wd.online?"online":"offline"}</span><span>KD20 {state?.snapshot?.kd20.online?"online":"offline"}</span><span>{printer?.online?"Nyomtató online":"Nyomtató offline"}</span></div></div>
          <div className="hhHeroBottom"><div className="hhStatusStrip"><button onClick={()=>chooseTab("people")}><small>Ki van itthon</small><strong>{presence.filter(p=>p.status==="home").length} fő</strong><span>{presence.filter(p=>p.status==="home").map(p=>p.name).slice(0,3).join(", ")||"Nincs biztos jelenlét"}</span></button><button onClick={()=>chooseTab("network")}><small>Hálózat</small><strong>{network.filter(n=>n.online).length}/{network.length}</strong><span>online eszköz</span></button><button onClick={()=>chooseTab("smart")}><small>Smart Life</small><strong>{onlineSmart}</strong><span>online</span></button><button onClick={()=>chooseTab("downloads")}><small>Letöltés</small><strong>{runningTorrents}</strong><span>aktív torrent</span></button><button onClick={()=>chooseTab("media")}><small>Média</small><strong>{media?.count||0}</strong><span>film</span></button><button onClick={()=>chooseTab("printer")}><small>Nyomtató</small><strong>{printer?.online?"OK":"—"}</strong><span>SCX-3200</span></button></div>
            <div className="hhFavorites"><span className="hhFavoritesTitle">Kedvencek</span><div><button onClick={()=>chooseTab("smart")}><i>⌂</i><span><b>Kapu</b><small>vezérlés</small></span></button><button onClick={()=>chooseTab("smart")}><i>❄</i><span><b>Klíma</b><small>állapot</small></span></button><button onClick={()=>chooseTab("downloads")}><i>↓</i><span><b>Torrent</b><small>hozzáadás</small></span></button><button onClick={()=>chooseTab("media")}><i>▶</i><span><b>Filmek</b><small>média</small></span></button><button onClick={()=>chooseTab("ai")}><i>✦</i><span><b>AI</b><small>asszisztens</small></span></button></div></div>
          </div>
        </div>
      </section>
      <PresenceStrip people={people} presence={presence} onOpen={()=>chooseTab("people")}/>{summaryCards}<section className="overviewGrid"><article className="panel overviewCard"><div className="sectionHead"><div><h2>Letöltések</h2><p>KD20 Transmission és WD automatikus másolás.</p></div><button className="ghost" onClick={()=>chooseTab("downloads")}>Megnyitás</button></div><div className="overviewStats"><span><b>{runningTorrents}</b> aktív letöltés</span><span><b>{torrents.filter(t=>t.percentDone>=1).length}</b> kész torrent</span><span><b>{state?.settings.autoCopyEnabled?"Be":"Ki"}</b> automatikus másolás</span></div></article><article className="panel overviewCard"><div className="sectionHead"><div><h2>Otthoni hálózat</h2><p>Két Wi-Fi és a vezetékes topológia.</p></div><button className="ghost" onClick={()=>chooseTab("network")}>Térkép</button></div><div className="ssidSummary"><span><i className="wifiIcon">⌁</i><b>krankovics2</b><small>Technicolor FGA2233</small></span><span><i className="wifiIcon">⌁</i><b>krankovics</b><small>Archer C6 + mesh</small></span></div></article><article className="panel overviewCard"><div className="sectionHead"><div><h2>Médiatár</h2><p>WD My Cloud filmek közvetlenül iPhone-ra.</p></div><button className="ghost" onClick={()=>chooseTab("media")}>Filmek</button></div><div className="overviewStats"><span><b>{media?.count||0}</b> film</span><span><b>{media?.online?"LAN":"Offline"}</b> helyi elérés</span><span><b>24 óra</b> aláírt link</span></div></article><article className="panel overviewCard"><div className="sectionHead"><div><h2>Smart Life</h2><p>Tuya és Xiaomi Home eszközök egyetlen nézetben.</p></div><button className="ghost" onClick={()=>chooseTab("smart")}>Eszközök</button></div><div className="overviewStats"><span><b>{onlineSmart}</b> online</span><span><b>{smart.devices.filter(d=>deviceKind(d)==="sensor").length}</b> szenzor</span><span><b>{smart.devices.filter(d=>deviceKind(d)==="climate").length}</b> klíma</span></div></article><article className="panel overviewCard"><div className="sectionHead"><div><h2>Nyomtató</h2><p>KD20 USB Print Server.</p></div><button className="ghost" onClick={()=>chooseTab("printer")}>Megnyitás</button></div><span className={printer?.online?"statusBadge good":"statusBadge"}>{printer?.online?"Nyomtatószolgáltatás elérhető":"Nincs észlelve"}</span></article></section></div>}

    {tab==="people"&&<PeopleTab people={people} presence={presence} network={network} reload={load} flash={flash} currentUser={currentUser}/>}
    ...
    {tab==="network"&&<div className="tabPanel"><section className="panel networkPanel networkHeroV17"><div className="sectionHead"><div><span className="smartEyebrowV12">NETWORK INTELLIGENCE</span><h2>Otthoni hálózat</h2><p>Élő topológia, Tuya MAC-alapú automatikus eszközazonosítás, IP-újrafelderítés és switch portmonitorozás a WD Bridge-ről.</p></div><div className="networkHealthSummary"><strong>{network.filter(n=>n.online).length}/{network.length}</strong><span>online</span><b>{network.flatMap(n=>n.managed?.ports||[]).filter(p=>p.linkUp&&p.speedMbps>0&&p.speedMbps<1000).length}</b><span>lassú link</span></div></div><NetworkTopology network={network}/></section><section className="panel networkPanel"><ManagedSwitchPanel network={network}/><NetworkIntegrationStatus network={network}/></section>{identityEditing&&<DeviceIdentityEditor device={identityEditing} onClose={()=>setIdentityEditing(null)} onSaved={load} flash={flash}/>}<section className="panel networkPanel"><div className="sectionHead"><div><h2>Élő eszközállapot</h2><p>Ping, ARP, MAC-alapú címfeloldás, Tuya factory-info és kézzel megerősített identitások. Az Azonosítás gomb az oldalon belüli szerkesztőt nyitja.</p></div><span className="sectionCounter">{network.filter(n=>n.online).length}/{network.length} online</span></div><div className="networkGrid">{network.map(n=><NetworkDeviceCard n={n} onIdentify={identifyNetwork} key={n.id}/>)}{network.length===0&&<div className="empty">A Bridge még nem küldött hálózati adatot.</div>}</div><div className="infoLine">A HomeHub a Tuya factory-info MAC-et, a megerősített családi eszközöket és a kézi azonosításokat használja. Azonos MAC több IP-n mesh proxyra utalhat; ezt nem kezeli új valódi eszközként.</div></section><section className="panel networkPanel"><NetworkEventTimeline events={state?.networkEvents||[]}/></section></div>}

    {tab==="credentials"&&<CredentialsTab vault={vault}/>}
    ...
    {tab==="settings"&&<div className="tabPanel"><PushNotificationSettings currentUser={currentUser} status={automation.notification} flash={flash} reload={load}/><Life360Settings status={state?.life360} people={people} flash={flash} reload={load}/><section className="panel settings settingsPanel"><div><h2>Torrent automatika</h2><p>A kész torrentet WD-re másolja, a KD20-on seedeléshez megőrzi. A beállítás a WD-n is tartósan mentődik.</p></div><label className="switch"><input type="checkbox" checked={state?.settings.autoCopyEnabled||false} onChange={e=>updateSettings({autoCopyEnabled:e.target.checked})}/><span></span> Automatikus másolás</label><label>Célmappa a WD-n<input value={state?.settings.autoCopyDestination||""} onChange={e=>setState(s=>s?({...s,settings:{...s.settings,autoCopyDestination:e.target.value}}):s)} onBlur={e=>updateSettings({autoCopyDestination:e.target.value})}/></label></section><section className="panel systemInfo"><div><span>Bridge</span><strong>{state?.bridgeOnline?"Online":"Offline"}</strong><small>Utolsó kapcsolat: {bridgeAge(state?.bridgeLastSeenAt)}</small></div><div><span>WD állapotmentés</span><strong>Aktív</strong><small>/DataVolume/homehub/server-state.json</small></div><div><span>Értesítések</span><strong>{[automation.notification.pushConfigured&&"Push",automation.notification.emailConfigured&&"Email",automation.notification.smsConfigured&&"SMS"].filter(Boolean).join(" · ")||"Nincs csatorna"}</strong><small>Személyhez kötött címzettek · opcionális eszkaláció</small></div><div><span>AI Asszisztens</span><strong>{ai.configured?`${ai.model} · ${state?.settings.aiMode==="approved"?"jóváhagyott":"javaslat"}`:"Nincs API kulcs"}</strong><small>OpenAI Responses API · szerveroldali kulcs · policy ellenőrzés</small></div><div><span>HomeHub</span><strong>v0.23.0</strong><small>User-alapú Notification Router · Web Push · SMS/Twilio · címzettek · eszkaláció · jelenléti mismatch trigger</small></div></section></div>}
  </main></div>
}
