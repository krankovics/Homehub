...
type Tab = "overview"|"downloads"|"media"|"smart"|"actions"|"ai"|"network"|"credentials"|"printer"|"settings";
...
function App(){
  ...
  const torrents=state?.snapshot?.kd20.torrents||[],printer=state?.snapshot?.printer,network=state?.snapshot?.network||[],vacuum=state?.snapshot?.vacuum,mediaSummary=state?.snapshot?.media,media=mediaLibrary??mediaSummary,vault=state?.snapshot?.vault,smart:SmartHome=state?.smartHome??{configured:false,online:false,lastUpdatedAt:null,devices:[],scenes:[]},automation:AutomationState=state?.automation??{rules:[],alerts:[],unread:0,email:{configured:false,recipients:0}},ai:AIState=state?.ai??{configured:false,model:"gpt-5",mode:state?.settings?.aiMode||"suggest",policy:"confirm-before-execute"};
