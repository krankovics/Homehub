...
type PersonProfile = {id:string;name:string;nickname?:string;role?:string;auth?:PersonAuthMeta;devices:PersonDeviceLink[];createdAt:string;updatedAt:string;hasAvatar?:boolean};
...
type AutomationRule = {id:string;name:string;enabled:boolean;trigger:AutomationTrigger;actions:AutomationAction[];cooldownSeconds:number;notifyEmail?:boolean;createdAt:string;updatedAt:string;lastTriggeredAt?:string};
...
type AIAutomationDraftResult = {explanation:string;draft:null|{name:string;enabled:boolean;trigger:AutomationTrigger;actions:AutomationAction[];cooldownSeconds:number;notifyEmail?:boolean};valid:boolean;warnings:string[];usage?:AIUsage};
...
async function api(path:string,init?:RequestInit){const r=await fetch(path,init);if(r.status===401)throw new Error("AUTH_REQUIRED");const b=await r.json().catch(()=>({}));if(r.status===403)throw new Error("FORBIDDEN");if(!r.ok)throw new Error(b?.error||`HTTP ${r.status}`);return b}
...
function triggerLabel(rule:AutomationRule,smart:SmartHome,network:NetworkStatus[]){
  ...
  if(t.type==="network.offline"){const n=network.find(x=>x.id===t.networkId);return `${n?.name||"Hálózati eszköz"} offline${t.forSeconds?` · ${Math.round(t.forSeconds/60)} percig`:""}`}
...
type ActionTemplate="temp_climate"|"gate_alert"|"machine_night"|"network_offline"|"slow_link"|"vacuum_schedule"|"new_device"|"ai_summary";
function AutomationComposer({smart,network,vacuum,onClose,onSaved,flash}:{smart:SmartHome;network:NetworkStatus[];vacuum?:VacuumStatus;onClose:()=>void;onSaved:()=>void;flash:(m:string)=>void}){
  const sensors=smart.devices.filter(d=>deviceKind(d)==="sensor"),climates=smart.devices.filter(d=>deviceKind(d)==="climate"),gates=smart.devices.filter(d=>deviceKind(d)==="gate"),machines=network.filter(n=>["computer","discovered"].includes(n.kind)),infra=network.filter(n=>["gateway","router","extender","switch","nas"].includes(n.kind)),managedSwitches=network.filter(n=>(n.managed?.ports||[]).length>0);
  ...
  const[vacTime,setVacTime]=useState("10:00");const[days,setDays]=useState<number[]>([1,2,3,4,5]);const[summaryTime,setSummaryTime]=useState("20:30");const[notifyEmail,setNotifyEmail]=useState(true);const[busy,setBusy]=useState(false);
  ...
  </div><div className="automationNotifyV21"><label><input type="checkbox" checked={notifyEmail} onChange={e=>setNotifyEmail(e.target.checked)}/><span></span><div><b>Email értesítés lefutáskor</b><small>Az ok, a végrehajtott akció és az eredmény is bekerül az emailbe és az Idővonalba.</small></div></label></div><div className="actionComposerFooter"><button className="ghost" onClick={onClose}>Mégse</button><button className="primaryAction" disabled={busy||template==="vacuum_schedule"&&days.length===0} onClick={save}>{busy?"Mentés…":"Akció létrehozása"}</button></div></section></div>
}
...
function PeopleTab({people,presence,network,reload,flash,currentUser}:{people:PersonProfile[];presence:PresenceStatus[];network:NetworkStatus[];reload:()=>void;flash:(m:string)=>void;currentUser:AuthUser}){
  const[editing,setEditing]=useState<PersonProfile|null>(null),[creating,setCreating]=useState(false),[name,setName]=useState(""),[nickname,setNickname]=useState(""),[role,setRole]=useState("Családtag"),[links,setLinks]=useState<Record<string,PersonDeviceRole|"">>({}),[busy,setBusy]=useState(false);
  ...
  function reset(){setEditing(null);setCreating(false);setName("");setNickname("");setRole("Családtag");setLinks({});setAccessEnabled(false);setLoginName("");setAccessPassword("");setPermissions(["overview"])}
  function startNew(){reset();setCreating(true)}
  function startEdit(p:PersonProfile){setEditing(p);setCreating(true);setName(p.name);setNickname(p.nickname||"");setRole(p.role||"Családtag");setLinks(Object.fromEntries(p.devices.map(d=>[d.networkId,d.role])));setAccessEnabled(Boolean(p.auth?.enabled));setLoginName(p.auth?.loginName||"");setAccessPassword("");setPermissions(p.auth?.permissions?.length?p.auth.permissions:["overview"])}
  function togglePermission(id:MenuPermission){setPermissions(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id])}
  async function save(){if(!name.trim())return;setBusy(true);try{
    const devices=Object.entries(links).filter(([,r])=>Boolean(r)).map(([networkId,r])=>({networkId,role:r as PersonDeviceRole,label:network.find(n=>n.id===networkId)?.name||""}));
    const body={name:name.trim(),nickname:nickname.trim(),role:role.trim(),devices};
    const saved=await api(editing?`/api/people/${editing.id}`:"/api/people",{method:editing?"PUT":"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)}) as PersonProfile;
    if(currentUser.isAdmin){
      const effectiveLogin=loginName.trim()||(accessEnabled?(nickname.trim()||name.trim()).toLocaleLowerCase("hu-HU").replace(/\s+/g,"."):"");
      await api(`/api/people/${saved.id}/access`,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({enabled:accessEnabled,loginName:effectiveLogin,password:accessPassword,permissions:permissions.length?permissions:["overview"]})});
    }
    flash(editing?"Profil és hozzáférés frissítve":"Személy felvéve");reset();reload()
  }catch(err){flash(`Profil hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}finally{setBusy(false)}}
...
function App(){
  ...
  const torrents=state?.snapshot?.kd20.torrents||[],printer=state?.snapshot?.printer,network=state?.snapshot?.network||[],vacuum=state?.snapshot?.vacuum,mediaSummary=state?.snapshot?.media,media=mediaLibrary??mediaSummary,vault=state?.snapshot?.vault,people=state?.people||[],presence=state?.presence||[],timeline=state?.timeline||[],smart:SmartHome=state?.smartHome??{configured:false,online:false,lastUpdatedAt:null,devices:[],scenes:[]},automation:AutomationState=state?.automation??{rules:[],alerts:[],unread:0,email:{configured:false,recipients:0}},ai:AIState=state?.ai??{configured:false,model:"gpt-5",mode:state?.settings?.aiMode||"suggest",policy:"confirm-before-execute"};
  ...
  async function identifyNetwork(n:NetworkStatus){
    if(!n.mac)return;
    const current=n.identity?.source==="manual"?n.name:"";
    const name=window.prompt("Eszköz neve",current||n.name||"");if(name===null||!name.trim())return;
    const kind=window.prompt("Eszköztípus (pl. telefon, TV beltéri, okoskonnektor)",n.identity?.deviceType||"");if(kind===null)return;
    const owner=window.prompt("Tulajdonos (opcionális)",n.identity?.owner||"");if(owner===null)return;
    try{await api(`/api/network/identity/${encodeURIComponent(n.mac)}`,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({name:name.trim(),kind:kind.trim(),owner:owner.trim(),note:"HomeHub felületen megerősítve."})});flash("Eszköz azonosítása mentve");load()}catch(err){flash(`Azonosítás hiba: ${err instanceof Error?err.message:"ismeretlen"}`)}
  }
  ...
  return <div className="hhApp">{notice&&<div className="toast">{notice}</div>}{deleteTarget&&<DeleteDialog torrent={deleteTarget} onClose={()=>setDeleteTarget(null)} onDelete={removeTorrent}/>} {resolvedDetail&&<DeviceDetailDialog device={resolvedDetail} onClose={()=>setDetailTarget(null)} onCommand={smartCommand}/>} {vacuumDetail&&vacuum&&<VacuumDetailDialog vacuum={vacuum} onClose={()=>setVacuumDetail(false)} onAction={vacuumAction}/>}
    ...
    {tab==="network"&&<div className="tabPanel"><section className="panel networkPanel networkHeroV17"><div className="sectionHead"><div><span className="smartEyebrowV12">NETWORK INTELLIGENCE</span><h2>Otthoni hálózat</h2><p>Élő topológia, Tuya MAC-alapú automatikus eszközazonosítás, IP-újrafelderítés és switch portmonitorozás a WD Bridge-ről.</p></div><div className="networkHealthSummary"><strong>{network.filter(n=>n.online).length}/{network.length}</strong><span>online</span><b>{network.flatMap(n=>n.managed?.ports||[]).filter(p=>p.linkUp&&p.speedMbps>0&&p.speedMbps<1000).length}</b><span>lassú link</span></div></div><NetworkTopology network={network}/></section><section className="panel networkPanel"><ManagedSwitchPanel network={network}/><NetworkIntegrationStatus network={network}/></section><section className="panel networkPanel"><div className="sectionHead"><div><h2>Élő eszközállapot</h2><p>Ping, ARP, MAC-alapú címfeloldás, Tuya factory-info és kézzel megerősített identitások.</p></div><span className="sectionCounter">{network.filter(n=>n.online).length}/{network.length} online</span></div><div className="networkGrid">{network.map(n=><NetworkDeviceCard n={n} onIdentify={identifyNetwork} key={n.id}/>)}{network.length===0&&<div className="empty">A Bridge még nem küldött hálózati adatot.</div>}</div><div className="infoLine">A HomeHub a Tuya factory-info MAC-et, a megerősített családi eszközöket és a kézi azonosításokat használja. Azonos MAC több IP-n mesh proxyra utalhat; ezt nem kezeli új valódi eszközként.</div></section><section className="panel networkPanel"><NetworkEventTimeline events={state?.networkEvents||[]}/></section></div>}
