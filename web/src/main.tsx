...
type NetworkStatus = { id:string; name:string; kind:string; online:boolean; adminOnline?:boolean; ip:string; configuredIp?:string; ipSource?:string; ipChanged?:boolean; mac:string; latencyMs:number; adminUrl:string; note:string; managed?:NetworkManagedStatus; visibility?:string };
...
function App(){
  ...
  return <main>{notice&&<div className="toast">{notice}</div>}{deleteTarget&&<DeleteDialog torrent={deleteTarget} onClose={()=>setDeleteTarget(null)} onDelete={removeTorrent}/>} {resolvedDetail&&<DeviceDetailDialog device={resolvedDetail} onClose={()=>setDetailTarget(null)} onCommand={smartCommand}/>} {vacuumDetail&&vacuum&&<VacuumDetailDialog vacuum={vacuum} onClose={()=>setVacuumDetail(false)} onAction={vacuumAction}/>}<header className="hero"><div><div className="eyebrow">HOME HUB · CORE</div><h1>Otthoni vezérlőközpont</h1><p>NAS, média, torrent, hálózat, nyomtató és okosotthon egyetlen PWA-ban.</p></div><div className="heroActions"><div className="live"><span className={state?.bridgeOnline?"dot on":"dot"}></span>{state?.bridgeOnline?"Bridge online":`Bridge offline · ${bridgeAge(state?.bridgeLastSeenAt)}`}</div><button className="ghost" onClick={logout}>Kilépés</button></div></header>
    ...
    {tab==="network"&&<div className="tabPanel"><section className="panel networkPanel networkHeroV17"><div className="sectionHead"><div><span className="smartEyebrowV12">NETWORK INTELLIGENCE</span><h2>Otthoni hálózat</h2><p>Élő topológia, automatikus IP-újrafelderítés és menedzselhető switch portmonitorozás a WD Bridge-ről.</p></div><div className="networkHealthSummary"><strong>{network.filter(n=>n.online).length}/{network.length}</strong><span>online</span><b>{network.flatMap(n=>n.managed?.ports||[]).filter(p=>p.linkUp&&p.speedMbps>0&&p.speedMbps<1000).length}</b><span>lassú link</span></div></div><NetworkTopology network={network}/></section><section className="panel networkPanel"><ManagedSwitchPanel network={network}/><NetworkIntegrationStatus network={network}/></section><section className="panel networkPanel"><div className="sectionHead"><div><h2>Élő eszközállapot</h2><p>Ping, ARP, MAC-alapú címfeloldás és admin-port ellenőrzés.</p></div><span className="sectionCounter">{network.filter(n=>n.online).length}/{network.length} online</span></div><div className="networkGrid">{network.map(n=><NetworkDeviceCard n={n} key={n.id}/>)}{network.length===0&&<div className="empty">A Bridge még nem küldött hálózati adatot.</div>}</div><div className="infoLine">A D-Link GO-SW-5G és a TP-Link LiteWave LS105G nem menedzselhető, ezért passzív topológiai elemként jelennek meg. Ha egy ismert MAC új DHCP-címet kap, a Bridge automatikusan az új címet használja.</div></section><section className="panel networkPanel"><NetworkEventTimeline events={state?.networkEvents||[]}/></section></div>}
