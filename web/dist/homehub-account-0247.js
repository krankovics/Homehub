(() => {
  ...
  function openSheet() {
    ...
    { const b=document.createElement("button"); b.type="button"; b.className="hhAccountActionV246"; b.innerHTML="<span>🔔</span><b>Értesítések</b><small>Push és riasztási előzmények</small>"; b.onclick=()=>navigate("notifications"); actions.appendChild(b); }
    if (can("settings")) {
      const b=document.createElement("button"); b.type="button"; b.className="hhAccountActionV246"; b.innerHTML="<span>⚙</span><b>Beállítások</b><small>Rendszer és fiók</small>"; b.onclick=()=>navigate("settings"); actions.appendChild(b);
    }
    if (can("people")) {
  ...
  function ensureSettingsCard() {
    ...
    if(!u || !settingsPanel || document.querySelector(".accountSettingsV246")) return;
    ...
    const exit=document.createElement("button");exit.type="button";exit.className="hhAccountLogoutV246";exit.dataset.hhAccountLogout="1";exit.textContent="Kilépés";exit.onclick=logout;actions.appendChild(exit);
    settingsPanel.parentElement?.insertBefore(card,settingsPanel);
  }
