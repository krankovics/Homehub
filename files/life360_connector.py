...
def main():
    ...
    # OTP/web-login accounts should provide LIFE360_ACCESS_TOKEN directly.
    # Keep the old username/password flow only as an optional fallback.
    if not token:
        if not user or not pw:
            raise RuntimeError(
                'LIFE360_ACCESS_TOKEN or LIFE360_USERNAME/LIFE360_PASSWORD missing'
            )
        t = req(
            HOST + '/v3/oauth2/token',
            'POST',
            {
                'grant_type': 'password',
                'username': user,
