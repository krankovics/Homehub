...
import type { HistoryEvent, NetworkEvent, NetworkStatus, PersonProfile, PresenceStatus, State } from "./types.js";

const RETENTION_MS = 90 * 24 * 60 * 60 * 1000;
const MAX_HISTORY = 10_000;

export function pushHistory(state: State, event: Omit<HistoryEvent, "id"> & { id?: string }) {
  const full: HistoryEvent = { id: event.id || crypto.randomUUID(), ...event };
...
export function networkEventsToHistory(events: NetworkEvent[]): HistoryEvent[] {
  return events.map((e) => ({
...
function networkById(network: NetworkStatus[]) {
  return new Map(network.map((n) => [n.id, n]));
}

export function derivePresence(person: PersonProfile, network: NetworkStatus[], previous?: PresenceStatus): PresenceStatus {
  ...
  let status: PresenceStatus["status"] = "uncertain";
  let confidence = 0;
...
export function updatePresence(state: State, network: NetworkStatus[]) {
  const now = new Date().toISOString();
  ...
  for (const e of events) pushHistory(state, e);
  return next;
}

export function recordHourlyNetworkSample(state: State, network: NetworkStatus[]) {
  const d = new Date();
  const key = d.toISOString().slice(0, 13);
  if (state.historySampleKey === key) return;
...
function statusMap(device: any) {
  return Object.fromEntries((Array.isArray(device?.status) ? device.status : []).map((x: any) => [String(x.code || ""), x.value]));
}

function gateValue(device: any) {
...
function boolValue(value: unknown) {
  ...
  if (["true","1","on","open","opened","enabled"].includes(v)) return true;
...
function normalizeGate(value: unknown) {
  ...
  if (["closed", "close", "zárt"].includes(v)) return "closed";
  if (["opened", "open", "nyitva", "opening", "partially opened", "partial open"].includes(v)) return "open";
  return v || "unknown";
}

export function tuyaDeviceHistory(beforeDevices: any[], afterDevices: any[]): HistoryEvent[] {
  const before = new Map(beforeDevices.map((d: any) => [String(d.id), d]));
  const events: HistoryEvent[] = [];
  const now = new Date().toISOString();
