...
type RawAutomationDraft = {
  ...
  explanation: string;
  trigger: {
    type: "tuya.numeric" | "tuya.state" | "network.online_window" | "network.new_device" | "schedule";
    deviceId: string | null;
    code: string | null;
    operator: "gt" | "gte" | "lt" | "lte" | "eq" | "neq" | "none";
    valueType: "number" | "string" | "boolean" | "none";
  ...
  actions: Array<{
    type: "tuya.command" | "vacuum.command" | "ai.summary" | "alert";
    deviceId: string | null;
    code: string | null;
    valueType: "number" | "string" | "boolean" | "none";
    numberValue: number | null;
    stringValue: string | null;
    booleanValue: boolean | null;
    vacuumAction: "start" | "pause" | "stop" | "dock" | "none";
    subject: string;
    message: string;
    email: boolean;
  }>;
...
export type AIActionPlan = {
  kind: "tuya.command" | "vacuum.command" | "none";
  summary: string;
  deviceId: string;
  code: string;
  valueType: "boolean" | "number" | "string" | "none";
...
function usage(payload: any): Usage {
...
function valueFrom(valueType: string, numberValue: number | null, stringValue: string | null, booleanValue: boolean | null): unknown {
...
function isGateText(v: string) { return /kapu|gate|garage|garázs|door|lock|zár/i.test(v); }
function isBlockedInstruction(code: string) { return /erase|learn|factory|reset|pair|credential|password/i.test(code); }
function isEVCurrentInstruction(code: string) { return /devicemaxseta|current.*set|set.*current|set\d+a|set60a|set80a/i.test(code); }

export class AIService {
  private lastCallAt = 0;

  constructor(private store: Store, private tuya: TuyaService) {}
  ...
  private ensureAvailable() {
    ...
    if (this.store.get().settings.aiMode === "off") throw new Error("ai_disabled");
  }

  private async responses(body: Record<string, unknown>) {
    this.ensureAvailable();
    const now = Date.now();
    if (now - this.lastCallAt < 700) await new Promise(r => setTimeout(r, 700 - (now - this.lastCallAt)));
  ...
  private context(includeFunctions = false) {
  ...
  async draftAutomation(request: string) {
    ...
    const raw = JSON.parse(text) as RawAutomationDraft;
    const warnings = [...(raw.warnings || [])];
    const trigger = this.convertTrigger(raw.trigger, warnings);
    const actions = raw.actions.map(a => this.convertAction(a, warnings)).filter(Boolean) as AutomationAction[];
    if (!trigger) warnings.push("A trigger nem alakítható érvényes HomeHub feltétellé.");
    if (!actions.length) warnings.push("Nincs biztonságosan végrehajtható akció a draftban.");
  ...
  private convertTrigger(t: RawAutomationDraft["trigger"], warnings: string[]): AutomationTrigger | null {
    if (t.type === "tuya.numeric") {
      if (!t.deviceId || !t.code || !["gt","gte","lt","lte","eq"].includes(t.operator) || t.numberValue === null) return null;
      return { type: t.type, deviceId: t.deviceId, code: t.code, operator: t.operator as "gt"|"gte"|"lt"|"lte"|"eq", value: t.numberValue, forSeconds: t.forSeconds || 0 };
    }
    if (t.type === "tuya.state") {
      if (!t.deviceId || !t.code || !["eq","neq"].includes(t.operator)) return null;
      const value = valueFrom(t.valueType, t.numberValue, t.stringValue, t.booleanValue);
      if (value === null) return null;
      return { type: t.type, deviceId: t.deviceId, code: t.code, operator: t.operator as "eq"|"neq", value: value as string|number|boolean, forSeconds: t.forSeconds || 0 };
    }
    if (t.type === "network.online_window") {
      if (!t.networkId || !t.after || !t.before) return null;
    ...
    if (t.type === "network.new_device") return { type: t.type };
    if (!t.time || !Array.isArray(t.days) || !t.days.length) return null;
    return { type: "schedule", time: t.time, days: [...new Set(t.days)].filter(d => d >= 0 && d <= 6), timezone: t.timezone || "Europe/Budapest" };
  }

  private convertAction(a: RawAutomationDraft["actions"][number], warnings: string[]): AutomationAction | null {
    if (a.type === "alert") {
    ...
    if (a.type === "vacuum.command") {
      if (a.vacuumAction === "none") return null;
      return { type: "vacuum.command", action: a.vacuumAction };
    }
    if (!a.deviceId || !a.code) return null;
    ...
    if (!d) { warnings.push(`HIBA: Tuya eszköz nem található: ${a.deviceId}`); return null; }
    if (d.profile === "mygate" || isGateText(`${d.name} ${d.productName}`)) { warnings.push("HIBA: Kapuvezérlési akciót az AI policy blokkol."); return null; }
    if (isBlockedInstruction(a.code)) { warnings.push(`HIBA: Telepítő/biztonsági DP blokkolva: ${a.code}`); return null; }
    if (d.profile === "feyree" && isEVCurrentInstruction(a.code)) { warnings.push(`HIBA: EV áramlimit DP blokkolva AI automatizálásból: ${a.code}`); return null; }
    return { type: "tuya.command", deviceId: a.deviceId, code: a.code, value };
  }

  private validateDraft(draft: { trigger: AutomationTrigger; actions: AutomationAction[] } | null, warnings: string[]) {
    if (!draft) return;
    ...
    if (trigger.type === "tuya.numeric" || trigger.type === "tuya.state") {
      ...
      if (!d) warnings.push(`HIBA: Trigger eszköz nem található: ${trigger.deviceId}`);
      else if (!d.status.some(s => s.code === trigger.code)) warnings.push(`HIBA: Trigger DP nincs az aktuális státuszban: ${trigger.code}`);
    }
    if (trigger.type === "network.online_window") {
      if (!(this.store.get().snapshot?.network || []).some(n => n.id === trigger.networkId)) warnings.push(`HIBA: Hálózati eszköz nem található: ${trigger.networkId}`);
    }
    for (const a of draft.actions) {
      if (a.type === "tuya.command") {
        const d = devices.find(x => x.id === a.deviceId);
        if (!d) warnings.push(`HIBA: Akció eszköz nem található: ${a.deviceId}`);
        else if (!d.functions.some(f => f.code === a.code)) warnings.push(`HIBA: A DP nem vezérelhető az aktuális function listában: ${a.code}`);
      }
      if (a.type === "vacuum.command") {
        const v = this.store.get().snapshot?.vacuum;
        if (!v?.configured || !v.controlReady) warnings.push("HIBA: A porszívó vezérlése még nincs készre konfigurálva.");
      }
    }
  }

  async draftAction(request: string) {
    const context = this.context(true);
    const data = await this.responses({
    ...
    const raw = JSON.parse(outputText(data) || "{}") as AIActionPlan;
  ...
  validateActionPlan(plan: AIActionPlan): { valid: boolean; plan: AIActionPlan; warning?: string } {
    if (!plan || plan.kind === "none") return { valid: false, plan, warning: plan?.reason || "Nincs végrehajtható művelet." };
    if (plan.kind === "vacuum.command") {
      const v = this.store.get().snapshot?.vacuum;
      if (!v?.configured || !v.online || !v.controlReady) return { valid: false, plan: { ...plan, risk: "blocked" }, warning: "A porszívó nem áll készen a vezérlésre." };
      if (plan.vacuumAction === "none") return { valid: false, plan, warning: "Hiányzik a porszívó művelet." };
    ...
    if (!d) return { valid: false, plan: { ...plan, risk: "blocked" }, warning: "A kiválasztott Tuya eszköz nem található." };
    if (d.profile === "mygate" || isGateText(`${d.name} ${d.productName}`)) return { valid: false, plan: { ...plan, risk: "blocked" }, warning: "Kapuvezérlés AI-ból blokkolva van." };
    if (isBlockedInstruction(plan.code)) return { valid: false, plan: { ...plan, risk: "blocked" }, warning: "Telepítő vagy reset DP AI-ból blokkolva van." };
    if (d.profile === "feyree" && isEVCurrentInstruction(plan.code)) return { valid: false, plan: { ...plan, risk: "blocked" }, warning: "EV áramlimit módosítása AI-ból blokkolva van." };
    if (!d.functions.some(f => f.code === plan.code)) return { valid: false, plan: { ...plan, risk: "blocked" }, warning: "A kiválasztott DP jelenleg nem vezérelhető." };
    return { valid: true, plan: { ...plan, risk: d.profile === "feyree" ? "medium" : plan.risk === "blocked" ? "medium" : plan.risk } };
  }

  actionValue(plan: AIActionPlan): unknown {
    if (plan.valueType === "boolean") return plan.booleanValue;
    if (plan.valueType === "number") return plan.numberValue;
    if (plan.valueType === "string") return plan.stringValue;
    return null;
  }
}
