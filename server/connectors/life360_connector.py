...
def main():
    # OTP-s Life360 fióknál ezt a tokent a
    # POST /v5/web/users/signin/otp/token válasz access_token mezőjéből kapjuk.
    token = os.getenv('LIFE360_ACCESS_TOKEN', '').strip()
    user = os.getenv('LIFE360_USERNAME', '').strip()
    pw = os.getenv('LIFE360_PASSWORD', '').strip()

    # Régi jelszavas flow csak fallbackként marad meg.
    if not token:
