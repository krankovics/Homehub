...
def main():
    user=os.getenv('LIFE360_USERNAME',''); pw=os.getenv('LIFE360_PASSWORD',''); token=os.getenv('LIFE360_ACCESS_TOKEN','')
    if not user or not pw: raise RuntimeError('LIFE360_USERNAME/LIFE360_PASSWORD missing')
    if not token:
        t=req(HOST+'/v3/oauth2/token','POST',{'grant_type':'password','username':user,'password':pw},'Basic '+CLIENT)
        token=t.get('access_token','')
        if not token: raise RuntimeError('Life360 login did not return access_token')
    auth='Bearer '+token
    ...
    if not circle: raise RuntimeError('No Life360 circle available')
    members=req(HOST+'/v3/circles/%s/members' % circle['id'],auth=auth).get('members',[])
    print(json.dumps({'accessToken':token,'circleId':circle.get('id'),'circleName':circle.get('name'),'members':members}, separators=(',',':')))
