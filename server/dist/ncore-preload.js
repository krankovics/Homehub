...
function adminSession(req) {
    ...
    if (userId !== "admin" || !/^\d+$/.test(exp || "") || Number(exp) < Math.floor(Date.now() / 1000))
        return false;
...
function requireBridge(req, res, next) {
    ...
    if (!BRIDGE_TOKEN || !safeEqual(auth, `Bearer ${BRIDGE_TOKEN}`))
...
function cleanupCommands() {
    ...
    for (let i = brokerCommands.length - 1; i >= 0; i--) {
        ...
        if (Number.isFinite(t) && t < cutoff)
            brokerCommands.splice(i, 1);
    }
    if (brokerCommands.length > 100)
        brokerCommands.splice(0, brokerCommands.length - 100);
}
...
async function waitForCommand(id) {
    ...
    while (Date.now() < deadline) {
        ...
        if (!cmd)
            throw new Error("ncore_bridge_command_lost");
        if (cmd.completedAt)
            return cmd;
...
async function fetchDirectRss() {
    ...
    if (!response.ok)
        throw new Error(`ncore_rss_http_${response.status}`);
    const head = body.slice(0, 12000);
    if (/just a moment|attention required|cf-chl-|cloudflare/i.test(head))
        throw new Error("ncore_rss_cloudflare");
    if (!/<(?:rss|feed)\b/i.test(body) && !/<item\b/i.test(body) && !/rss_dl\.php\/id=/i.test(body))
        throw new Error("ncore_rss_invalid_response");
    return body;
}
function parseDirectRss(xml) {
    const results = [];
...
async function fallbackSearch(query, category, bridgeError = "") {
...
async function directTorrentFile(id) {
    if (!PASSKEY)
        throw new Error("ncore_passkey_missing");
    ...
    if (!response.ok)
        throw new Error(`ncore_download_http_${response.status}`);
    if (/text\/html/i.test(type) || /just a moment|attention required|cf-chl-|cloudflare/i.test(head))
        throw new Error("ncore_download_cloudflare");
    if (!data.length || data.length > 20 * 1024 * 1024 || data[0] !== 0x64)
        throw new Error("ncore_invalid_torrent_file");
    return data;
}
function errorStatus(message) {
    if (message === "ncore_session_expired")
        return 401;
    if (message === "ncore_bridge_credentials_missing" || message === "ncore_bridge_offline" || message === "ncore_bridge_timeout")
        return 503;
    if (message === "ncore_cloudflare" || message === "ncore_download_cloudflare")
        return 503;
    if (message === "invalid_torrent_id" || message === "search_too_short")
        return 400;
    return 502;
}
function register(app) {
    if (app.__homehubNcoreRegistered)
        return;
    app.__homehubNcoreRegistered = true;
    app.get("/api/ncore/bridge/commands", requireBridge, (req, res) => {
        ...
        const available = brokerCommands.filter((cmd) => {
            if (cmd.bridgeId !== bridgeId || cmd.completedAt)
                return false;
            if (!cmd.leasedAt)
                return true;
        ...
        for (const cmd of available)
            cmd.leasedAt = new Date().toISOString();
        res.json(available.map(({ id, type, payload }) => ({ id, type, payload })));
    });
    app.post("/api/ncore/bridge/commands/:id/complete", requireBridge, express.json({ limit: "2mb" }), (req, res) => {
    ...
    app.get("/api/ncore/status", requireAdmin, (_req, res) => {
    ...
    app.get("/api/ncore/search", requireAdmin, async (req, res) => {
        if (!ENABLED)
            return res.status(503).json({ error: "ncore_disabled" });
        const q = String(req.query?.q || "").trim().slice(0, 120);
        const category = ["all", "movies", "tv"].includes(String(req.query?.category || "all")) ? String(req.query?.category || "all") : "all";
        if (q.length < 2)
            return res.status(400).json({ error: "search_too_short" });
        if (bridgeOnline() && bridgeState.configured && !bridgeSearchBlocked()) {
            try {
                ...
                if (!done.ok)
                    throw new Error(done.message || "ncore_bridge_search_failed");
    ...
    app.post("/api/ncore/add/:id", requireAdmin, async (req, res) => {
        if (!ENABLED)
            return res.status(503).json({ error: "ncore_disabled" });
        const id = String(req.params?.id || "");
        if (!/^\d{1,12}$/.test(id))
        ...
        if (!bridgeOnline())
            return res.status(503).json({ error: "ncore_bridge_offline" });
        if (!bridgeState.configured)
            return res.status(503).json({ error: "ncore_bridge_credentials_missing" });
        try {
            const cmd = enqueue("ncore.download", { id });
            const done = await waitForCommand(cmd.id);
            if (!done.ok)
                throw new Error(done.message || "ncore_bridge_download_failed");
            let detail = {};
            try {
                detail = JSON.parse(done.message || "{}");
            }
            catch { }
    ...
    app.get("/api/ncore/torrent/:id", requireAdmin, async (req, res) => {
        const id = String(req.params?.id || "");
        if (!/^\d{1,12}$/.test(id))
            return res.status(400).json({ error: "invalid_torrent_id" });
        try {
