...
const normMac = (v) => v.trim().toLowerCase().replace(/-/g, ":");
const normText = (v) => String(v ?? "").trim().toLowerCase().replace(/[_-]+/g, " ");
function fmtLocal(now, timezone = DEFAULT_TZ) {
...
function hm(v) { const m = /^(\d{1,2}):(\d{2})$/.exec(v.trim()); return m ? Math.max(0, Math.min(1439, Number(m[1]) * 60 + Number(m[2]))) : 0; }
function inWindow(nowMinutes, after, before) { const a = hm(after), b = hm(before); return a === b ? true : a < b ? nowMinutes >= a && nowMinutes < b : nowMinutes >= a || nowMinutes < b; }
function specMeta(d, code) { const s = [...d.functions, ...d.statusSpec].find(x => x.code === code); try {
    return JSON.parse(s?.values || "{}");
}
catch {
    return {};
} }
function numericValue(d, code) {
    ...
    if ((!scale || scale === 0) && /va_temperature/i.test(code) && Math.abs(value) > 100)
        value /= 10;
    return value;
}
function compareNumber(actual, op, expected) { if (op === "gt")
    return actual > expected; if (op === "gte")
    return actual >= expected; if (op === "lt")
    return actual < expected; if (op === "lte")
    return actual <= expected; return Math.abs(actual - expected) < 1e-7; }
function compareState(actual, op, expected) { const same = (typeof actual === "string" || typeof expected === "string") ? normText(actual) === normText(expected) : actual === expected; return op === "eq" ? same : !same; }
function operatorLabel(operator) { return { gt: ">", gte: "≥", lt: "<", lte: "≤", eq: "=", neq: "≠" }[operator] || operator; }
export class AutomationEngine {
    ...
    generateAISummary;
    ticking = false;
    constructor(store, tuya, mailer, notifier, queueVacuum, generateAISummary) {
        this.store = store;
        this.tuya = tuya;
        this.mailer = mailer;
        this.notifier = notifier;
        this.queueVacuum = queueVacuum;
        ...
        try {
            ...
            for (const rule of [...this.store.get().automations].filter(r => r.enabled)) {
                try {
                    ...
                    await this.logEngineError(rule, err);
                }
            }
        }
        finally {
    ...
    async runNow(rule) { await this.execute(rule, { detail: "Kézi teszt / futtatás" }); }
    runtime(ruleId) { return this.store.get().automationRuntime[ruleId] || {}; }
    updateRuntime(ruleId, patch) { this.store.mutate(s => { s.automationRuntime[ruleId] = { ...(s.automationRuntime[ruleId] || {}), ...patch }; }); }
    cooldownOK(rule, now) { if (!rule.lastTriggeredAt)
        return true; const last = new Date(rule.lastTriggeredAt).getTime(); return !Number.isFinite(last) || now - last >= Math.max(0, rule.cooldownSeconds || 0) * 1000; }
    legacyPlan(rule, emailRequested = true) {
        if (rule.notification)
            return rule.notification;
        if (rule.notifyEmail === false || !emailRequested)
            return { enabled: false, priority: "normal", recipientPersonIds: [], channels: [] };
        return { enabled: true, priority: "normal", recipientPersonIds: [], channels: ["email"], fallbackToAdmin: true };
    }
    async captureNewNetworkDevices() {
        ...
        for (const rule of this.store.get().automations.filter(r => r.enabled && r.trigger.type === "network.new_device")) {
            if (!this.cooldownOK(rule, Date.now()))
                continue;
    ...
    evaluateCondition(trigger, now) {
        ...
        if (trigger.type === "signal.state" || trigger.type === "signal.numeric") {
            ...
            if (signal && freshByExpiry && freshByAge) {
                if (trigger.type === "signal.numeric")
                    active = typeof signal.value === "number" && compareNumber(signal.value, trigger.operator, trigger.value);
                else
                    active = compareState(signal.value, trigger.operator, trigger.value);
            }
    ...
    async evaluate(rule) {
        if (rule.trigger.type === "network.new_device")
            return;
        ...
        if (rule.trigger.type === "schedule") {
            ...
            if (!days.includes(local.weekday) || local.time !== rule.trigger.time)
                return;
            const key = `${local.date}@${rule.trigger.time}`;
            if (rt.lastScheduleKey === key || !this.cooldownOK(rule, nowMs))
                return;
        ...
        if (!active) {
            if (rt.conditionSince || rt.latched || rt.triggeredAt)
                this.updateRuntime(rule.id, { conditionSince: undefined, latched: false, triggeredAt: undefined, escalationsSent: [] });
            return;
        }
        if (rt.latched) {
            await this.processEscalations(rule, nowMs, detail);
            return;
        }
        const since = rt.conditionSince ? new Date(rt.conditionSince).getTime() : nowMs;
        if (!rt.conditionSince)
            this.updateRuntime(rule.id, { conditionSince: now.toISOString() });
        if (nowMs - since < forSeconds * 1000 || !this.cooldownOK(rule, nowMs))
            return;
        const triggeredAt = now.toISOString();
        this.updateRuntime(rule.id, { latched: true, triggeredAt, escalationsSent: [] });
    ...
    async processEscalations(rule, nowMs, detail) {
        const plan = rule.notification, rt = this.runtime(rule.id);
        if (!plan?.enabled || !plan.escalations?.length || !rt.triggeredAt)
            return;
        ...
        for (let i = 0; i < plan.escalations.length; i++) {
            const step = plan.escalations[i];
            if (sent.has(i) || elapsed < step.afterSeconds)
                continue;
            ...
            sent.add(i);
            this.updateRuntime(rule.id, { escalationsSent: [...sent] });
        }
    }
    actionSummary(action) { if (action.type === "vacuum.command")
        return `Porszívó: ${action.action}`; if (action.type === "tuya.command") {
        ...
        return "AI összefoglaló"; return `Értesítés: ${action.subject}`; }
    render(template, rule, detail) { return template.replaceAll("{{rule}}", rule.name).replaceAll("{{detail}}", detail).replaceAll("{{time}}", new Date().toLocaleString("hu-HU", { timeZone: DEFAULT_TZ })); }
    async execute(rule, context) {
        const failures = [], performed = [];
        for (const action of rule.actions) {
            try {
                await this.executeAction(rule, action, context.detail);
                performed.push(this.actionSummary(action));
            }
            catch (err) {
                failures.push(`${this.actionSummary(action)}: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        ...
        this.store.mutate(s => { const target = s.automations.find(x => x.id === rule.id); if (target)
            target.lastTriggeredAt = firedAt; pushHistory(s, { category: "automation", type: failures.length ? "automation.partial" : "automation.executed", entityId: rule.id, entityName: rule.name, message: historyMessage, createdAt: firedAt, data: { reason, actions: performed, failures, result } }); });
        ...
        if (!hasExplicitAlert && (rule.notification?.enabled || rule.notifyEmail !== false)) {
            ...
            if (failures.length)
                lines.push("", "Hibák:", failures.join("\n"));
    ...
    async executeAction(rule, action, detail) {
        ...
        if (action.type === "tuya.command") {
            const d = this.tuya.state().devices.find(x => x.id === action.deviceId);
            if (!d)
                throw new Error("tuya_device_not_found");
            const gateLike = d.profile === "mygate" || /kapu|gate|garage|garázs|door|lock|zár/i.test(`${d.name} ${d.productName}`);
            if (gateLike && !rule.safety?.allowGateAction)
                throw new Error("gate_action_requires_explicit_safety_opt_in");
            if (gateLike && /^(?:start_1|stop_1|pedestrian_1|open_1|close_1|gate_(?:open|close|start|stop)|door_(?:open|close))$/i.test(action.code) && action.value !== true)
                throw new Error("gate_pulse_must_be_true");
            await this.tuya.command(action.deviceId, action.code, action.value);
            return;
        }
        if (action.type === "ai.summary") {
            const subject = this.render(action.subject || "HomeHub AI összefoglaló", rule, detail), message = await this.generateAISummary();
            await this.createAlert(rule, subject, message, action.email !== false);
            return;
        }
        if (action.type === "alert") {
            const subject = this.render(action.subject || rule.name, rule, detail), message = this.render(action.message || "{{detail}}", rule, detail);
            await this.createAlert(rule, subject, message, action.email !== false);
        }
    }
    async createAlert(rule, subject, message, notifyRequested, channelsOverride, escalationLevel) {
        ...
        const record = { id: crypto.randomUUID(), ruleId: rule.id, ruleName: rule.name, subject, message, createdAt, priority: plan?.priority, emailRequested: Boolean(notifyRequested && (channelsOverride || plan?.channels || []).includes("email")), emailSent, emailError, deliveries, escalationLevel };
        this.store.mutate(s => {
            s.alerts.push(record);
            if (s.alerts.length > 200)
                s.alerts = s.alerts.slice(-200);
    ...
    async logEngineError(rule, err) { await this.createAlert(rule, "HomeHub automatizálási hiba", err instanceof Error ? err.message : String(err), false); }
}
