...
function embeddedMac(deviceId) {
    ...
    if (!id.startsWith("20284731") && !tail.startsWith("3c6105"))
        return "";
...
].map(([mac, v]) => [normalizeMac(mac), v]));
function identityFromOverride(o) {
    return {
        source: "manual",
        confidence: 100,
        label: o.name,
...
function identityFromKnown(mac, k) {
    return {
        source: "known_device",
        confidence: 100,
        label: k.label,
...
function tuyaMacCandidates(d) {
    return [...new Set([normalizeMac(d.mac), embeddedMac(d.id)].filter(x => /^[0-9a-f]{2}(?::[0-9a-f]{2}){5}$/.test(x)))];
}
export function enrichNetworkIdentities(network, tuyaDevices, overrides = {}) {
    const tuyaByMac = new Map();
    for (const d of tuyaDevices) {
        const factoryMac = normalizeMac(d.mac);
        if (/^[0-9a-f]{2}(?::[0-9a-f]{2}){5}$/.test(factoryMac))
            tuyaByMac.set(factoryMac, { device: d, source: "tuya_factory" });
        for (const mac of tuyaMacCandidates(d))
            if (!tuyaByMac.has(mac))
                tuyaByMac.set(mac, { device: d, source: "tuya_device_id" });
    }
    ...
    return network.map((n) => {
        ...
        if (override)
            identity = identityFromOverride(override);
        else if (KNOWN[mac])
            identity = identityFromKnown(mac, KNOWN[mac]);
